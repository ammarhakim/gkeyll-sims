...
integratedVThermSqElcWrite = DataStruct.DynVector { numComponents = 1, }
integratedVThermSqIonWrite = DataStruct.DynVector { numComponents = 1, }
...
function rk3(tCurr, myDt)
  ...
  statusElc = dragStatusElc and diffStatusElc
  statusIon = dragStatusIon and diffStatusIon
  dtSuggestedElc = math.min(dragDtSuggestedElc, diffDtSuggestedElc)
  dtSuggestedIon = math.min(dragDtSuggestedIon, diffDtSuggestedIon)

  if (statusElc == false) or (statusIon == false) then
    return false, math.min(dtSuggestedElc, dtSuggestedIon)
  end

  distf1Elc:applyPeriodicBc(0)
  distf1Ion:applyPeriodicBc(0)
  calcMoments(0.0, 0.0, distf1Elc, distf1Ion)

  -- RK stage 2
  distfNewElc:copy(distf1Elc)
...
function writeFields(frameNum, tCurr)
   ...
   mom2ParaElc:write( string.format("mom2ParaElc_%d.h5", frameNum), tCurr)
   mom2ParaIon:write( string.format("mom2ParaIon_%d.h5", frameNum), tCurr)
   mom1MuElc:write( string.format("mom1MuElc_%d.h5", frameNum), tCurr)
   mom1MuIon:write( string.format("mom1MuIon_%d.h5", frameNum), tCurr)
   driftUElc:write( string.format("driftUElc_%d.h5", frameNum), tCurr)
...
calcMoments(0.0, 0.0, distfElc, distfIonUnit)
numDensityIon:write( string.format("numDensityIonUnit_%d.h5", 0.0), 0.0)

-- apply boundary conditions
distfElc:applyPeriodicBc(0)
distfIon:applyPeriodicBc(0)
-- calc moments
calcMoments(0.0, 0.0, distfElc, distfIon)
-- scale ion distribution function so N_ELC = N_ION
