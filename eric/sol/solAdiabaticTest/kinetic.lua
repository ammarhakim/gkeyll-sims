-- Adiabatic electron test with positivity
...
function calcDiagnostics(curr, dt)
   ...
   runUpdater(heatFluxAtEdgeCalc, curr, dt, {firstMoment, thirdMoment, vThermSq, phi1dDiscont,
     momentsAtEdges}, {heatFluxAtEdge})
...
distfPositive = DataStruct.Field2D {
   onGrid = grid,
   numComponents = basis:numNodes(),
   ghost = {1, 1},
}
...
function rk3(tCurr, myDt)
   ...
   -- Enforce positivity
   enforcePositivity(positivityCalcIon, tCurr, myDt, distf1, distfPositive);
   -- Copy fields
   distf1:copy(distfPositive);
...
-- Enforce positivity
enforcePositivity(positivityCalcIon, 0, 0, distf, distfPositive);
-- Copy fields
distf:copy(distfPositive);
