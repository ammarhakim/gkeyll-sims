...
initDistf = Updater.EvalOnNodes2D {
   ...
   -- function to use for initialization
   evaluate = function(x,y,z,t)
     local backgroundTemp = 100 + 45/2 + 30*lSource/(lParallel*math.pi)
     local backgroundDens = (0.7 + 0.3/2 + 0.5*lSource/(lParallel*math.pi))*1e19
		 return backgroundDens*maxwellian(math.sqrt(backgroundTemp*eV/ionMass), 0.0, x, y)
	      end
}
