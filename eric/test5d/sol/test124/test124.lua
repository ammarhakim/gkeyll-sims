...
tStart = 0.0
tEnd = 9000e-6
dtSuggested = 1.6e-9--0.1*tEnd -- initial time-step to use (will be adjusted)
nFrames = 9000
...
grid_elc_5d = Grid.NonUniformRectCart5D {
   ...
   upper = {X_UPPER, Y_UPPER, Z_UPPER, VPARA_UPPER_ELC, 1},
   ...
   mappings = {
     ...
     function(zeta)
       return zeta^2*MU_UPPER_ELC
     end,
   },
   decomposition = phaseDecomp,
}

grid_ion_5d = Grid.NonUniformRectCart5D {
   lower = {X_LOWER, Y_LOWER, Z_LOWER, VPARA_LOWER_ION, 0},
   upper = {X_UPPER, Y_UPPER, Z_UPPER, VPARA_UPPER_ION, 1},
   cells = {N_X, N_Y, N_Z, N_VPARA, N_MU},
   mappings = {
     function(zeta)
       return zeta
...
diffSlvrElc = Updater.LenardBernsteinDiffAlternate3D2VUpdater {
  onGrid = grid_elc_5d,
  basis5d = basis_elc_5d,
  basis3d = basis_3d,
  basis2d = basis_2d,
...
diffSlvrIon = Updater.LenardBernsteinDiffAlternate3D2VUpdater {
  onGrid = grid_ion_5d,
  basis5d = basis_ion_5d,
  basis3d = basis_3d,
  basis2d = basis_2d,
...
diffSlvrElcIon = Updater.LenardBernsteinDiffAlternate3D2VUpdater {
  onGrid = grid_elc_5d,
  basis5d = basis_elc_5d,
  basis3d = basis_3d,
  basis2d = basis_2d,
...
densityBeforeElc = DataStruct.Field3D {
   onGrid = grid_3d,
   numComponents = basis_3d:numNodes(),
   ghost = {1, 1},
}

floorSourceElc = DataStruct.Field3D {
   onGrid = grid_3d,
   numComponents = basis_3d:numNodes(),
   ghost = {1, 1},
}

densityAfterElc = DataStruct.Field3D {
   onGrid = grid_3d,
   numComponents = basis_3d:numNodes(),
   ghost = {1, 1},
}

densityStage1Elc = DataStruct.Field3D {
   onGrid = grid_3d,
   numComponents = basis_3d:numNodes(),
   ghost = {1, 1},
}
densityStage2Elc = DataStruct.Field3D {
   onGrid = grid_3d,
   numComponents = basis_3d:numNodes(),
   ghost = {1, 1},
}
densityStage3Elc = DataStruct.Field3D {
   onGrid = grid_3d,
   numComponents = basis_3d:numNodes(),
   ghost = {1, 1},
}
...
function applyDensityFloor(tCurr, myDt, distfElc, distfIon)
  ...
  runUpdater(generalIntegralElcCalc, tCurr, myDt, {distfElc, oneFieldElc, bField3d}, {densityBeforeElc})
  runUpdater(generalIntegralIonCalc, tCurr, myDt, {distfIon, oneFieldIon, bField3d}, {numDensityIon})
  ...
  runUpdater(fieldInt3d, tCurr, myDt, {densityBeforeElc}, {beforeFloorElcNumberVec})

  -- enforce a density floor in the domain for electrons
  runUpdater(enforceDensityFloorElc, tCurr, myDt, {fEpsilonElc, densityBeforeElc, numDensityIon}, {distfElc})
  -- enforce a density floor in the domain for ions
  runUpdater(enforceDensityFloorIon, tCurr, myDt, {fEpsilonIon, numDensityIon, densityBeforeElc}, {distfIon})
  ...
  runUpdater(generalIntegralElcCalc, tCurr, myDt, {distfElc, oneFieldElc, bField3d}, {densityAfterElc})
  runUpdater(fieldInt3d, tCurr, myDt, {densityAfterElc}, {afterFloorElcNumberVec})
...
function rk3test(tCurr, myDt)
  ...
  local myStatusLBIon = true
  local myDtSuggestedLBIon = 100
  ...
  Lucee.logInfo(string.format("Floor source rate = %g", stage1NumberRate))
...
function rk3(tCurr, myDt)
  ...
  local stage1ElcFloorEnergy, stage1IonFloorEnergy, stage1NumberRate = applyDensityFloor(tCurr, myDt, f1Elc, f1Ion)
  densityStage1Elc:combine(1.0/myDt, densityAfterElc, -1.0/myDt, densityBeforeElc)
  ...
  densityStage2Elc:combine(1.0/myDt, densityAfterElc, -1.0/myDt, densityBeforeElc)
  ...
  densityStage3Elc:combine(1.0/myDt, densityAfterElc, -1.0/myDt, densityBeforeElc)
...
function calcDiagnostics(tCurr, myDt)
  ...
  floorSourceElc:combine(1/6, densityStage1Elc, 2/3, densityStage2Elc, 1, densityStage3Elc)
end
...
function writeFields(frameNum, tCurr)
  ...
  floorSourceElc:write( string.format("floorSourceElc_%d.h5", frameNum), tCurr)
  ...
  if Lucee.getRank() == 0 then
    ...
    os.execute("mv ".. filename .. "_" .. string.format("floorSourceElc_%d.h5", frameNum) .. " heatingElc")
