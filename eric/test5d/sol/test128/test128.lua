...
-- mpiexec -n 4 /Users/eshi/Research/gkeyllall/par-opt/gkeyll/gkeyll -i test128.lua -pc_type lu -pc_factor_mat_solver_package superlu_dist
filename = "test128"
...
function sourceDensityProfile(x,y,z)
  ...
  local lambdaSource = 0.04 --[m]
  local xSource = -0.03 + R0 + a0 --[m]

  if math.abs(z) < L_parallel/4 then
    if x < xSource then
      return sourceFloor
    else
      return math.exp(-(x-xSource)/lambdaSource)
    end
  else
    return 0
  end
