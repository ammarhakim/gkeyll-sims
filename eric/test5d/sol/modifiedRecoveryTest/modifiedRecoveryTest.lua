...
-- mpiexec -n 1 /Users/eshi/Research/gkeyllall/par-opt/gkeyll/gkeyll -i modifiedRecoveryTest.lua -pc_type lu -pc_factor_mat_solver_package superlu_dist
filename = "modifiedRecoveryTest"
...
initfElc = Updater.EvalOnNodes5D {
   ...
   evaluate = function (x,y,z,v,mu,t)
		 ...
		 local v_m = math.sqrt(elcTemp/6*eV*3/elcMass)
		 local mu_m = 2*elcTemp/6*eV/B0
...
function calcLenardBernsteinElcElc(tCurr, myDt, fInitial, fFinal)
  ...
  runUpdater(diffSlvrElc, tCurr, myDt, {fInitial, temperatureElc,
  ...
  local diffStatus, diffDtSuggested = runUpdater(lenardBernsteinScaleElcUpdater, tCurr, myDt, {fElcCollisions, elcEnergyAtCellDrag,
    elcEnergyAtCellDiff, temperatureElc, bField3d, numDensityElc}, {fFinal})

  if (diffStatus == false or dragStatus == false) then
    return false, math.min(dragDtSuggested, diffDtSuggested)
  end

  runUpdater(elcEnergyAtCellCalc, tCurr, myDt, {fFinal, jacobianFieldElc, hamilKeElc}, {kineticEnergyElc})
