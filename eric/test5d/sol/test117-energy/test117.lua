...
function elcTempProfile(x,y,z)
  return 6.5*initBackgroundProfile(x,y,z)
end
...
function fIonProfile(x,y,z,v,mu)
  return math.exp(-ionMass*v^2/(2*0.9*ionTempProfile(x,y,z)*eV))*
    math.exp(-math.abs(mu)*bFieldProfile(x)/(0.9*ionTempProfile(x,y,z)*eV))
...
runUpdater(initfIon, 0.0, 0.0, {}, {fEpsilonIon})
fEpsilonIon:sync()
...
function sourceTempProfile(x,y,z)
  ...
  local a = deltaX/2
  local edgeVal = 1/4
  if r <= a then --and math.abs(z) < L_parallel/4 then
    return (1-edgeVal)*(1-r^2/a^2)^3 + edgeVal
  else
    return edgeVal
  end
  --initBackgroundProfile(x,y,z)
end
...
function sourceDensityProfile(x,y,z)
  ...
  --if math.abs(z) < L_parallel/4 then
    return sourceFloor + (1-sourceFloor)*0.5*(1 - tanh( (r-r_s)/L_s ))
  --else
  --  return 0
  --end
end
...
initSourceIon = Updater.EvalOnNodes5D {
   ...
   -- function to use for initialization
   evaluate = function (x,y,z,v,mu,t)
		 return math.exp(-ionMass*v^2/(2*0.9*ionTemp*eV))*
      math.exp(-math.abs(mu)*bFieldProfile(x)/(0.9*ionTemp*eV))
	 end
}
...
function getElcAlpha(t)
  ...
  return 1/10*logLambda*elcCharge^4/(6*math.sqrt(2)*math.pi^(3/2)*eps0^2*math.sqrt(elcMass))
end
...
function applyDensityFloor(tCurr, myDt, distfElc, distfIon)
  ...
  runUpdater(elcEnergyAtCellCalc, tCurr, myDt, {distfElc, jacobianFieldElc, hamilKeElc}, {kineticEnergyElc})
...
function calcPotential(nIon, nElc, phiOut)
  ...
  local n_i0 = 6.0e17--avgIonDensity:lastInsertedData()/totalVol--2.325e17
  -- solve for potential
...
function calcHamiltonianConsElc(phiIn, hamilOut)
  -- copy phi3d into a 5d field
  runUpdater(copy3dTo5dElc, 0.0, 0.0, {phiIn}, {hamilOut})
  hamilOut:scale(0.5*elcCharge)
  -- compute second order hamiltonian correction
  --calcSecondOrderHamiltonian(phiIn, secondHamil3d)
  --runUpdater(copy3dTo5dElc, 0.0, 0.0, {secondHamil3d}, {secondHamil5dElc})
  --hamilOut:accumulate(-0.5*elcMass/B0^2,secondHamil5dElc)
...
function calcHamiltonianConsIon(phiIn, hamilOut)
  -- copy phi3d into a 5d field
  runUpdater(copy3dTo5dIon, 0.0, 0.0, {phiIn}, {hamilOut})
  hamilOut:scale(0.5*ionCharge)
  -- compute second order hamiltonian correction
  --calcSecondOrderHamiltonian(phiIn, secondHamil3d)
  --runUpdater(copy3dTo5dIon, 0.0, 0.0, {secondHamil3d}, {secondHamil5dIon})
  --hamilOut:accumulate(-0.5*ionMass/B0^2,secondHamil5dIon)
...
-- To keep track of energy loss terms
elcPowerLoss_1 = DataStruct.DynVector { numComponents = 2, }
elcPowerLoss_2 = DataStruct.DynVector { numComponents = 2, }
elcPowerLoss_3 = DataStruct.DynVector { numComponents = 2, }
ionPowerLoss_1 = DataStruct.DynVector { numComponents = 2, }
ionPowerLoss_2 = DataStruct.DynVector { numComponents = 2, }
ionPowerLoss_3 = DataStruct.DynVector { numComponents = 2, }
elcPowerLoss_4 = DataStruct.DynVector { numComponents = 2, }
ionPowerLoss_4 = DataStruct.DynVector { numComponents = 2, }
-- To keep track of source power added terms
elcPowerSource_1 = DataStruct.DynVector { numComponents = 1, }
elcPowerSource_2 = DataStruct.DynVector { numComponents = 1, }
elcPowerSource_3 = DataStruct.DynVector { numComponents = 1, }
ionPowerSource_1 = DataStruct.DynVector { numComponents = 1, }
ionPowerSource_2 = DataStruct.DynVector { numComponents = 1, }
ionPowerSource_3 = DataStruct.DynVector { numComponents = 1, }
-- To keep of energy before and after time step
initialElcEnergy = DataStruct.DynVector { numComponents = 1, }
initialIonEnergy = DataStruct.DynVector { numComponents = 1, }
finalElcEnergy = DataStruct.DynVector { numComponents = 1, }
finalIonEnergy = DataStruct.DynVector { numComponents = 1, }
-- To keep of kinetic energy before and after time step
initialElcKineticEnergy = DataStruct.DynVector { numComponents = 1, }
initialIonKineticEnergy = DataStruct.DynVector { numComponents = 1, }
finalElcKineticEnergy = DataStruct.DynVector { numComponents = 1, }
finalIonKineticEnergy = DataStruct.DynVector { numComponents = 1, }
-- The dynvectors that are written to disk
elcPowerLoss = DataStruct.DynVector { numComponents = 1, }
elcPowerSource = DataStruct.DynVector { numComponents = 1, }
elcPowerPositivity = DataStruct.DynVector { numComponents = 1, }
elcPowerFloor = DataStruct.DynVector { numComponents = 1, }
ionPowerLoss = DataStruct.DynVector { numComponents = 1, }
ionPowerSource = DataStruct.DynVector { numComponents = 1, }
ionPowerPositivity = DataStruct.DynVector { numComponents = 1, }
ionPowerFloor = DataStruct.DynVector { numComponents = 1, }
elcPowerDelta = DataStruct.DynVector { numComponents = 1, }
ionPowerDelta = DataStruct.DynVector { numComponents = 1, }
elcKineticPowerDelta = DataStruct.DynVector { numComponents = 1, }
ionKineticPowerDelta = DataStruct.DynVector { numComponents = 1, }
-- Placeholder field for integration
tempField3d = DataStruct.Field3D {
   onGrid = grid_3d,
   numComponents = basis_3d:numNodes(),
   ghost = {1, 1},
}
tempIonField5d = DataStruct.Field5D {
   onGrid = grid_ion_5d,
   numComponents = basis_ion_5d:numNodes(),
   ghost = {1, 1},
}
tempElcField5d = DataStruct.Field5D {
   onGrid = grid_elc_5d,
   numComponents = basis_elc_5d:numNodes(),
   ghost = {1, 1},
}
...
function rk3(tCurr, myDt)
  -- Compute initial conserved energy
  -- Construct H = H_KE + 0.5*q*phi
  calcHamiltonianConsElc(phi3d, tempElcField5d)
  calcHamiltonianConsIon(phi3d, tempIonField5d)
  --runUpdater(generalIntegralElcCalc, tCurr, myDt, {fElc, hamilElc, bField3d}, {kineticEnergyElc})
  runUpdater(elcEnergyAtCellCalc, tCurr, myDt, {fElc, jacobianFieldElc, tempElcField5d}, {kineticEnergyElc})
  runUpdater(fieldInt3d, tCurr, myDt, {kineticEnergyElc}, {initialElcEnergy})
  runUpdater(elcEnergyAtCellCalc, tCurr, myDt, {fElc, jacobianFieldElc, hamilKeElc}, {kineticEnergyElc})
  runUpdater(fieldInt3d, tCurr, myDt, {kineticEnergyElc}, {initialElcKineticEnergy})
  --runUpdater(generalIntegralIonCalc, tCurr, myDt, {fIon, hamilIon, bField3d}, {kineticEnergyIon})
  runUpdater(ionEnergyAtCellCalc, tCurr, myDt, {fIon, jacobianFieldIon, tempIonField5d}, {kineticEnergyIon})
  runUpdater(fieldInt3d, tCurr, myDt, {kineticEnergyIon}, {initialIonEnergy})
  runUpdater(ionEnergyAtCellCalc, tCurr, myDt, {fIon, jacobianFieldIon, hamilKeIon}, {kineticEnergyIon})
  runUpdater(fieldInt3d, tCurr, myDt, {kineticEnergyIon}, {initialIonKineticEnergy})
  -- Compute energy lost
  --runUpdater(heatFluxAtEdgeElcCalc, tCurr, myDt, {fElc,hamilDerivKeElc,hamilElc,bField3d},
  --  {heatFluxLowerElc, heatFluxUpperElc,elcPowerLoss_1})
  runUpdater(multiply5dCalcElc, tCurr, myDt, {fElc,hamilDerivKeElc}, {tempElcField5d})
  runUpdater(totalEdgeFluxElcCalc, tCurr, myDt, {tempElcField5d, jacobianFieldElc, hamilElc},
    {elcPowerLoss_1})
  --runUpdater(heatFluxAtEdgeIonCalc, tCurr, myDt, {fIon,hamilDerivKeIon,hamilIon,bField3d},
  --  {heatFluxLowerIon, heatFluxUpperIon,ionPowerLoss_1})
  runUpdater(multiply5dCalcIon, tCurr, myDt, {fIon,hamilDerivKeIon}, {tempIonField5d})
  runUpdater(totalEdgeFluxIonCalc, tCurr, myDt, {tempIonField5d, jacobianFieldIon, hamilIon},
    {ionPowerLoss_1})
  -- Compute source power
  --runUpdater(generalIntegralElcCalc, tCurr, myDt, {elcSource, hamilElc, bField3d}, {kineticEnergyElc})
  runUpdater(elcEnergyAtCellCalc, tCurr, myDt, {elcSource, jacobianFieldElc, hamilElc}, {kineticEnergyElc})
  runUpdater(fieldInt3d, tCurr, myDt, {kineticEnergyElc}, {elcPowerSource_1})
  --runUpdater(generalIntegralIonCalc, tCurr, myDt, {ionSource, hamilIon, bField3d}, {kineticEnergyIon})
  runUpdater(ionEnergyAtCellCalc, tCurr, myDt, {ionSource, jacobianFieldIon, hamilIon}, {kineticEnergyIon})
  runUpdater(fieldInt3d, tCurr, myDt, {kineticEnergyIon}, {ionPowerSource_1})
  -- RK stage 1
  ...
  if (netStatus == true) then
    initialElcEnergy:removeLastInsertedData()
    initialIonEnergy:removeLastInsertedData()
    elcPowerSource_1:removeLastInsertedData()
    ionPowerSource_1:removeLastInsertedData()
    elcPowerLoss_1:removeLastInsertedData()
    ionPowerLoss_1:removeLastInsertedData()
  ...
  -- Compute energy lost
  --runUpdater(heatFluxAtEdgeElcCalc, tCurr, myDt, {f1Elc,hamilDerivKeElc,hamilElc,bField3d},
  --  {heatFluxLowerElc, heatFluxUpperElc,elcPowerLoss_2})
  runUpdater(multiply5dCalcElc, tCurr, myDt, {f1Elc,hamilDerivKeElc}, {tempElcField5d})
  runUpdater(totalEdgeFluxElcCalc, tCurr, myDt, {tempElcField5d, jacobianFieldElc, hamilElc},
    {elcPowerLoss_2})
  --runUpdater(heatFluxAtEdgeIonCalc, tCurr, myDt, {f1Ion,hamilDerivKeIon,hamilIon,bField3d},
  --  {heatFluxLowerIon, heatFluxUpperIon,ionPowerLoss_2})
  runUpdater(multiply5dCalcIon, tCurr, myDt, {f1Ion,hamilDerivKeIon}, {tempIonField5d})
  runUpdater(totalEdgeFluxIonCalc, tCurr, myDt, {tempIonField5d, jacobianFieldIon, hamilIon},
    {ionPowerLoss_2})
  -- Compute source power
  --runUpdater(generalIntegralElcCalc, tCurr, myDt, {elcSource, hamilElc, bField3d}, {kineticEnergyElc})
  runUpdater(elcEnergyAtCellCalc, tCurr, myDt, {elcSource, jacobianFieldElc, hamilElc}, {kineticEnergyElc})
  runUpdater(fieldInt3d, tCurr, myDt, {kineticEnergyElc}, {elcPowerSource_2})
  --runUpdater(generalIntegralIonCalc, tCurr, myDt, {ionSource, hamilIon, bField3d}, {kineticEnergyIon})
  runUpdater(ionEnergyAtCellCalc, tCurr, myDt, {ionSource, jacobianFieldIon, hamilIon}, {kineticEnergyIon})
  runUpdater(fieldInt3d, tCurr, myDt, {kineticEnergyIon}, {ionPowerSource_2})
  ...
  if (netStatus == true) then
    ...
    elcPowerLoss_1:removeLastInsertedData()
    ionPowerLoss_1:removeLastInsertedData()
    elcPowerSource_2:removeLastInsertedData()
    ionPowerSource_2:removeLastInsertedData()
    elcPowerLoss_2:removeLastInsertedData()
    ionPowerLoss_2:removeLastInsertedData()
    return false, netDtSuggested
  end
  -- add on particle source
  ...
  -- Compute energy lost
  --runUpdater(heatFluxAtEdgeElcCalc, tCurr, myDt, {f1Elc,hamilDerivKeElc,hamilElc,bField3d},
  --  {heatFluxLowerElc, heatFluxUpperElc,elcPowerLoss_3})
  runUpdater(multiply5dCalcElc, tCurr, myDt, {f1Elc,hamilDerivKeElc}, {tempElcField5d})
  runUpdater(totalEdgeFluxElcCalc, tCurr, myDt, {tempElcField5d, jacobianFieldElc, hamilElc},
    {elcPowerLoss_3})
  --runUpdater(heatFluxAtEdgeIonCalc, tCurr, myDt, {f1Ion,hamilDerivKeIon,hamilIon,bField3d},
  --  {heatFluxLowerIon, heatFluxUpperIon,ionPowerLoss_3})
  runUpdater(multiply5dCalcIon, tCurr, myDt, {f1Ion,hamilDerivKeIon}, {tempIonField5d})
  runUpdater(totalEdgeFluxIonCalc, tCurr, myDt, {tempIonField5d, jacobianFieldIon, hamilIon},
    {ionPowerLoss_3})
  -- Compute source power
  --runUpdater(generalIntegralElcCalc, tCurr, myDt, {elcSource, hamilElc, bField3d}, {kineticEnergyElc})
  runUpdater(elcEnergyAtCellCalc, tCurr, myDt, {elcSource, jacobianFieldElc, hamilElc}, {kineticEnergyElc})
  runUpdater(fieldInt3d, tCurr, myDt, {kineticEnergyElc}, {elcPowerSource_3})
  --runUpdater(generalIntegralIonCalc, tCurr, myDt, {ionSource, hamilIon, bField3d}, {kineticEnergyIon})
  runUpdater(ionEnergyAtCellCalc, tCurr, myDt, {ionSource, jacobianFieldIon, hamilIon}, {kineticEnergyIon})
  runUpdater(fieldInt3d, tCurr, myDt, {kineticEnergyIon}, {ionPowerSource_3})
  ...
  if (netStatus == true) then
    ...
    elcPowerLoss_2:removeLastInsertedData()
    ionPowerLoss_2:removeLastInsertedData()
    elcPowerSource_3:removeLastInsertedData()
    ionPowerSource_3:removeLastInsertedData()
    elcPowerLoss_3:removeLastInsertedData()
    ionPowerLoss_3:removeLastInsertedData()
    return false, netDtSuggested
  end
  -- add on particle source
  fNewElc:accumulate(myDt, elcSource)
  ...
  --runUpdater(generalIntegralElcCalc, tCurr, myDt, {fElc, hamilElc, bField3d}, {kineticEnergyElc})
  runUpdater(elcEnergyAtCellCalc, tCurr, myDt, {fElc, jacobianFieldElc, tempElcField5d}, {kineticEnergyElc})
  runUpdater(fieldInt3d, tCurr, myDt, {kineticEnergyElc}, {finalElcEnergy})
  runUpdater(elcEnergyAtCellCalc, tCurr, myDt, {fElc, jacobianFieldElc, hamilKeElc}, {kineticEnergyElc})
  runUpdater(fieldInt3d, tCurr, myDt, {kineticEnergyElc}, {finalElcKineticEnergy})
  --runUpdater(generalIntegralIonCalc, tCurr, myDt, {fIon, hamilIon, bField3d}, {kineticEnergyIon})
  runUpdater(ionEnergyAtCellCalc, tCurr, myDt, {fIon, jacobianFieldIon, tempIonField5d}, {kineticEnergyIon})
  runUpdater(fieldInt3d, tCurr, myDt, {kineticEnergyIon}, {finalIonEnergy})
  runUpdater(ionEnergyAtCellCalc, tCurr, myDt, {fIon, jacobianFieldIon, hamilKeIon}, {kineticEnergyIon})
  runUpdater(fieldInt3d, tCurr, myDt, {kineticEnergyIon}, {finalIonKineticEnergy})

  -- Since there is no code to directly put a new element into a dynvector, copy desired data to a field and
  -- ask field to be integrated, which will end up in a vector
  -- 1. Physical Sources
  local writeElcPowerSource = 1/6*elcPowerSource_1:lastInsertedData() + 1/6*elcPowerSource_2:lastInsertedData() + 2/3*elcPowerSource_3:lastInsertedData() 
  tempField3d:clear(writeElcPowerSource/totalVol)
  runUpdater(fieldInt3d, tCurr, myDt, {tempField3d}, {elcPowerSource})
  local writeIonPowerSource = 1/6*ionPowerSource_1:lastInsertedData() + 1/6*ionPowerSource_2:lastInsertedData() + 2/3*ionPowerSource_3:lastInsertedData()
  tempField3d:clear(writeIonPowerSource/totalVol)
  runUpdater(fieldInt3d, tCurr, myDt, {tempField3d}, {ionPowerSource})
  -- 2. Loss terms through sheath
  local elcPowerLossLower_1, elcPowerLossUpper_1 = elcPowerLoss_1:lastInsertedData()
  local elcPowerLossLower_2, elcPowerLossUpper_2 = elcPowerLoss_2:lastInsertedData()
  local elcPowerLossLower_3, elcPowerLossUpper_3 = elcPowerLoss_3:lastInsertedData()
  local ionPowerLossLower_1, ionPowerLossUpper_1 = ionPowerLoss_1:lastInsertedData()
  local ionPowerLossLower_2, ionPowerLossUpper_2 = ionPowerLoss_2:lastInsertedData()
  local ionPowerLossLower_3, ionPowerLossUpper_3 = ionPowerLoss_3:lastInsertedData()
  local writeElcPowerLoss = 1/6*(elcPowerLossUpper_1-elcPowerLossLower_1) + 1/6*(elcPowerLossUpper_2-elcPowerLossLower_2) + 2/3*(elcPowerLossUpper_3-elcPowerLossLower_3)
  tempField3d:clear(writeElcPowerLoss/totalVol)
  runUpdater(fieldInt3d, tCurr, myDt, {tempField3d}, {elcPowerLoss})
  local writeIonPowerLoss = 1/6*(ionPowerLossUpper_1-ionPowerLossLower_1) + 1/6*(ionPowerLossUpper_2-ionPowerLossLower_2) + 2/3*(ionPowerLossUpper_3-ionPowerLossLower_3)
  tempField3d:clear(writeIonPowerLoss/totalVol)
  runUpdater(fieldInt3d, tCurr, myDt, {tempField3d}, {ionPowerLoss})
  -- 3. Positivity Power
  local positivityPowerElc = stage3ElcEnergy + 2/3*stage2ElcEnergy + 1/6*stage1ElcEnergy
  tempField3d:clear(positivityPowerElc/totalVol)
  runUpdater(fieldInt3d, tCurr, myDt, {tempField3d}, {elcPowerPositivity})
  local positivityPowerIon = stage3IonEnergy + 2/3*stage2IonEnergy + 1/6*stage1IonEnergy
  tempField3d:clear(positivityPowerIon/totalVol)
  runUpdater(fieldInt3d, tCurr, myDt, {tempField3d}, {ionPowerPositivity})
  -- 4. Floor Power
  local floorPowerElc = stage3ElcFloorEnergy + 2/3*stage2ElcFloorEnergy + 1/6*stage1ElcFloorEnergy
  tempField3d:clear(floorPowerElc/totalVol)
  runUpdater(fieldInt3d, tCurr, myDt, {tempField3d}, {elcPowerFloor})
  local floorPowerIon = stage3IonFloorEnergy + 2/3*stage2IonFloorEnergy + 1/6*stage1IonFloorEnergy
  tempField3d:clear(floorPowerIon/totalVol)
  runUpdater(fieldInt3d, tCurr, myDt, {tempField3d}, {ionPowerFloor})
  -- 5. Change in energy
  local powerDeltaElc = (finalElcEnergy:lastInsertedData() - initialElcEnergy:lastInsertedData())/myDt
  tempField3d:clear(powerDeltaElc/totalVol)
  runUpdater(fieldInt3d, tCurr, myDt, {tempField3d}, {elcPowerDelta})
  local powerDeltaIon = (finalIonEnergy:lastInsertedData() - initialIonEnergy:lastInsertedData())/myDt
  tempField3d:clear(powerDeltaIon/totalVol)
  runUpdater(fieldInt3d, tCurr, myDt, {tempField3d}, {ionPowerDelta})
  -- 6. Change in kinetic energy
  local powerKineticDeltaElc = (finalElcKineticEnergy:lastInsertedData() - initialElcKineticEnergy:lastInsertedData())/myDt
  tempField3d:clear(powerKineticDeltaElc/totalVol)
  runUpdater(fieldInt3d, tCurr, myDt, {tempField3d}, {elcKineticPowerDelta})
  local powerKineticDeltaIon = (finalIonKineticEnergy:lastInsertedData() - initialIonKineticEnergy:lastInsertedData())/myDt
  tempField3d:clear(powerKineticDeltaIon/totalVol)
  runUpdater(fieldInt3d, tCurr, myDt, {tempField3d}, {ionKineticPowerDelta})

  Lucee.logInfo(string.format("Total elc energy added from positivity = %g", positivityPowerElc))
  Lucee.logInfo(string.format("Total elc energy added from floor = %g", floorPowerElc))
  Lucee.logInfo(string.format("Total ion energy added from positivity = %g", positivityPowerIon))
  Lucee.logInfo(string.format("Total ion energy added from floor = %g", floorPowerIon))
...
function calcDiagnostics(tCurr, myDt)
  ...
  runUpdater(heatFluxAtEdgeElcCalc, tCurr, myDt, {fElc,hamilDerivKeElc,hamilElc,bField3d},
    {heatFluxLowerElc, heatFluxUpperElc, elcPowerLoss_4})
  runUpdater(heatFluxAtEdgeIonCalc, tCurr, myDt, {fIon,hamilDerivKeIon,hamilIon,bField3d},
    {heatFluxLowerIon, heatFluxUpperIon, ionPowerLoss_4})
...
function writeFields(frameNum, tCurr)
  ...
  -- write data for energy vectors
  elcPowerLoss:write( string.format("elcPowerLoss_%d.h5", frameNum), tCurr)
  ionPowerLoss:write( string.format("ionPowerLoss_%d.h5", frameNum), tCurr)
  elcPowerSource:write( string.format("elcPowerSource_%d.h5", frameNum), tCurr)
  ionPowerSource:write( string.format("ionPowerSource_%d.h5", frameNum), tCurr)
  elcPowerPositivity:write( string.format("elcPowerPositivity_%d.h5", frameNum), tCurr)
  ionPowerPositivity:write( string.format("ionPowerPositivity_%d.h5", frameNum), tCurr)
  elcPowerFloor:write( string.format("elcPowerFloor_%d.h5", frameNum), tCurr)
  ionPowerFloor:write( string.format("ionPowerFloor_%d.h5", frameNum), tCurr)
  elcPowerDelta:write( string.format("elcPowerDelta_%d.h5", frameNum), tCurr)
  ionPowerDelta:write( string.format("ionPowerDelta_%d.h5", frameNum), tCurr)
  elcKineticPowerDelta:write( string.format("elcKineticPowerDelta_%d.h5", frameNum), tCurr)
  ionKineticPowerDelta:write( string.format("ionKineticPowerDelta_%d.h5", frameNum), tCurr)
  finalElcEnergy:write( string.format("elcTotalEnergy_%d.h5", frameNum), tCurr)
  finalIonEnergy:write( string.format("ionTotalEnergy_%d.h5", frameNum), tCurr)

  -- TESTING
  --elcPowerLoss_1:write( string.format("elcPowerLoss_1_%d.h5", frameNum), tCurr)
  --elcPowerLoss_2:write( string.format("elcPowerLoss_2_%d.h5", frameNum), tCurr)
  --elcPowerLoss_3:write( string.format("elcPowerLoss_3_%d.h5", frameNum), tCurr)
  --ionPowerLoss_1:write( string.format("ionPowerLoss_1_%d.h5", frameNum), tCurr)
  --ionPowerLoss_2:write( string.format("ionPowerLoss_2_%d.h5", frameNum), tCurr)
  --ionPowerLoss_3:write( string.format("ionPowerLoss_3_%d.h5", frameNum), tCurr)
  ...
  if Lucee.getRank() == 0 then
    ...
    os.execute("mv ".. filename .. "_" .. string.format("elcPowerLoss_%d.h5", frameNum) .. " dynvector")
    os.execute("mv ".. filename .. "_" .. string.format("ionPowerLoss_%d.h5", frameNum) .. " dynvector")
    os.execute("mv ".. filename .. "_" .. string.format("elcPowerSource_%d.h5", frameNum) .. " dynvector")
    os.execute("mv ".. filename .. "_" .. string.format("ionPowerSource_%d.h5", frameNum) .. " dynvector")
    os.execute("mv ".. filename .. "_" .. string.format("elcPowerPositivity_%d.h5", frameNum) .. " dynvector")
    os.execute("mv ".. filename .. "_" .. string.format("ionPowerPositivity_%d.h5", frameNum) .. " dynvector")
    os.execute("mv ".. filename .. "_" .. string.format("elcPowerFloor_%d.h5", frameNum) .. " dynvector")
    os.execute("mv ".. filename .. "_" .. string.format("ionPowerFloor_%d.h5", frameNum) .. " dynvector")
    os.execute("mv ".. filename .. "_" .. string.format("elcPowerDelta_%d.h5", frameNum) .. " dynvector")
    os.execute("mv ".. filename .. "_" .. string.format("ionPowerDelta_%d.h5", frameNum) .. " dynvector")
    os.execute("mv ".. filename .. "_" .. string.format("elcKineticPowerDelta_%d.h5", frameNum) .. " dynvector")
    os.execute("mv ".. filename .. "_" .. string.format("ionKineticPowerDelta_%d.h5", frameNum) .. " dynvector")
    os.execute("mv ".. filename .. "_" .. string.format("ionTotalEnergy_%d.h5", frameNum) .. " dynvector")
    os.execute("mv ".. filename .. "_" .. string.format("elcTotalEnergy_%d.h5", frameNum) .. " dynvector")

    --os.execute("mv ".. filename .. "_" .. string.format("elcPowerLoss_1_%d.h5", frameNum) .. " dynvector")
    --os.execute("mv ".. filename .. "_" .. string.format("elcPowerLoss_2_%d.h5", frameNum) .. " dynvector")
    --os.execute("mv ".. filename .. "_" .. string.format("elcPowerLoss_3_%d.h5", frameNum) .. " dynvector")
    --os.execute("mv ".. filename .. "_" .. string.format("ionPowerLoss_1_%d.h5", frameNum) .. " dynvector")
    --os.execute("mv ".. filename .. "_" .. string.format("ionPowerLoss_2_%d.h5", frameNum) .. " dynvector")
    --os.execute("mv ".. filename .. "_" .. string.format("ionPowerLoss_3_%d.h5", frameNum) .. " dynvector")
...
Lucee.logInfo(string.format( 'Source particle rate = %g [1/(s*m^3)]',totalSourceElc ))
