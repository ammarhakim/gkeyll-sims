...
-- stores result of potential solve
phi3dBack = DataStruct.Field3D {
   onGrid = grid_3d,
   numComponents = basis_3d:numNodes(),
   ghost = {1, 1},
}
-- stores result of potential solve
phi3dFluct = DataStruct.Field3D {
   onGrid = grid_3d,
   numComponents = basis_3d:numNodes(),
   ghost = {1, 1},
}
...
phiFluctSqVec = DataStruct.DynVector { numComponents = 1, }
...
function calcContinuousDiagnostics(tCurr, myDt)
  ...
  -- Compute (phi-phiBack)^2 integrated in system
  phi3dFluct:combine(1.0, phi3d, -1.0, phi3dBack)
  runUpdater(weightedFieldInt3d, tCurr, myDt, {phi3dFluct, phi3dFluct}, {phiFluctSqVec})
end
...
function writeFields(frameNum, tCurr)
  ...
  phiFluctSqVec:write( string.format("phiFluctSqVec_%d.h5", frameNum), tCurr )
  ...
  if Lucee.getRank() == 0 then
    ...
    os.execute("mv ".. filename .. "_" .. string.format("phiFluctSqVec_%d.h5", frameNum) .. " dynvector")
