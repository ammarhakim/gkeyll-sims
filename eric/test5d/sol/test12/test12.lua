...
-- mpiexec -n 1 /Users/eshi/Research/gkeyllall/par-opt/gkeyll/gkeyll -i test12.lua -pc_type lu -pc_factor_mat_solver_package superlu_dist
...
-- 4-7-16: testing guiding center number conservation
...
initHamilKeElc = Updater.EvalOnNodes5D {
   ...
   evaluate = function (x,y,z,vPara,mu,t)
      return 0.5*elcMass*vPara*vPara + mu*bFieldProfile(x)
   end
}
...
initHamilKeIon = Updater.EvalOnNodes5D {
   ...
   shareCommonNodes = false,
   evaluate = function (x,y,z,vPara,mu,t)
      return 0.5*ionMass*vPara*vPara + mu*bFieldProfile(x)
   end
}
...
-- to help track down flux issue
ionFluxCalc = Updater.SOLFluxAcrossEdgeCalc {
  onGrid = grid_ion_5d,
  basis5d = basis_ion_5d,
  basis3d = basis_3d,
...
-- stores integrated outward flux on both surfaces
totalIonFlux = DataStruct.DynVector { numComponents = 2, }

elcFluxCalc = Updater.SOLFluxAcrossEdgeCalc {
  onGrid = grid_elc_5d,
  basis5d = basis_elc_5d,
  basis3d = basis_3d,
...
-- stores integrated outward flux on both surfaces
totalElcFlux = DataStruct.DynVector { numComponents = 2, }
...
function applyBcToDistF(tCurr, myDt, fElc, fIon)
  ...
  runUpdater(edgeMomentCalcIon, tCurr, myDt, {fIon, hamilKeIon}, {edgeMomentsIon})
  -- apply logical sheath boundary conditions to electron distribution function
  sheathStatus = runUpdater(logicalSheathCalc, tCurr, myDt, {edgeMomentsIon, hamilKeElc}, {fElc, phiSLower, phiSUpper})
...
function calcDiagnostics(tCurr, myDt)
  ...
  -- compute fluxes on both surfaces
  runUpdater(ionFluxCalc, tCurr, myDt, {fIon, bField3d}, {totalIonFlux})
  local ionFluxLower, ionFluxUpper = totalIonFlux:lastInsertedData()
  runUpdater(elcFluxCalc, tCurr, myDt, {fElc, bField3d}, {totalElcFlux})
  local elcFluxLower, elcFluxUpper = totalElcFlux:lastInsertedData()
  local totalSurfaceArea = (X_UPPER-X_LOWER)*(Y_UPPER-Y_LOWER)
  Lucee.logInfo( string.format("Lower Gamma_e = %.15g \nLower Gamma_i = %.15g \n",elcFluxLower, ionFluxLower) )
  Lucee.logInfo( string.format("Upper Gamma_e = %.15g \nUpper Gamma_i = %.15g \n",elcFluxUpper, ionFluxUpper) )

  -- calculate first vPar moment of the electron distribution function
  runUpdater(edgeMomentCalcElc, tCurr, myDt, {fElc, hamilKeElc}, {edgeMomentsElc})
...
function writeFields(frameNum, tCurr)
  ...
  edgeMomentsIon:scale(2*math.pi*B0/ionMass)
  edgeMomentsIon:write( string.format("edgeMomentsIon_%d.h5", frameNum), tCurr)
  edgeMomentsElc:scale(2*math.pi*B0/elcMass)
