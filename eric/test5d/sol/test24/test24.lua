...
-- mpiexec -n 4 /Users/eshi/Research/gkeyllall/par-opt/gkeyll/gkeyll -i test25.lua -pc_type lu -pc_factor_mat_solver_package superlu_dist
...
edgeMomentCalcIonWithGhost = Updater.MomentsAtEdges5D {
  basis5d = basis_ion_5d,
  basis3d = basis_3d,
  onGrid = grid_ion_5d,
  polyOrder = polyOrder,
  integrateGhosts = true, -- can be false, but use true to make sure fIon = 0 in ghosts
}
...
edgeMomentCalcElcWithGhost = Updater.MomentsAtEdges5D {
  basis5d = basis_elc_5d,
  basis3d = basis_3d,
  onGrid = grid_elc_5d,
  polyOrder = polyOrder,
...
function rk3(tCurr, myDt)
  ...
  if gcError > 1e-8 then
    endExecution = true
    Lucee.logInfo (string.format("*** RK1 Ending execution because guiding center error is %g", gcError))
    return false, math.min(myDtSuggestedElc, myDtSuggestedIon)
  end
  ...
  -- calculate density of each species for potential solve
  runUpdater(numDensityElcCalc, tCurr, myDt, {fNewElc, bField3d}, {numDensityElc})
  numDensityElc:scale(2*math.pi/elcMass)
  runUpdater(numDensityIonCalc, tCurr, myDt, {fNewIon, bField3d}, {numDensityIon})
  numDensityIon:scale(2*math.pi/ionMass)
  ...
  runUpdater(fieldInt3d, tCurr, myDt, {numDensityIon}, {totalIonCounter})
  -- compute relative error in guiding center density
  gcError = math.abs(totalElcCounter:lastInsertedData()-totalIonCounter:lastInsertedData())/totalElcCounter:lastInsertedData()
  if gcError > 1e-8 then
    endExecution = true
    Lucee.logInfo (string.format("*** RK2a Ending execution because guiding center error is %g", gcError))
    fElc:copy(f1Elc)
    fIon:copy(f1Ion)
    return false, math.min(myDtSuggestedElc, myDtSuggestedIon)
  end
  ...
  if gcError > 1e-8 then
    endExecution = true
    Lucee.logInfo (string.format("*** RK2b Ending execution because guiding center error is %g", gcError))
    return false, math.min(myDtSuggestedElc, myDtSuggestedIon)
  end
  ...
  if gcError > 1e-8 then
    endExecution = true
    Lucee.logInfo (string.format("*** RK3 Ending execution because guiding center error is %g", gcError))
    return false, math.min(myDtSuggestedElc, myDtSuggestedIon)
  end
...
function calcDiagnostics(tCurr, myDt)
  ...
  runUpdater(edgeMomentCalcElcWithGhost, tCurr, myDt, {fElc, hamilDerivKeElc}, {edgeMomentsElc})
  runUpdater(edgeMomentCalcIonWithGhost, tCurr, myDt, {fIon, hamilDerivKeIon}, {edgeMomentsIon})
...
if Lucee.IsRestarting then
  ...
  -- heat flux at edge
  runUpdater(heatFluxAtEdgeIonCalc, tCurr, 0, {fIon, hamilDerivKeIon, hamilIon, bField3d},
    {heatFluxLowerIon, heatFluxUpperIon})
  runUpdater(heatFluxAtEdgeElcCalc, tCurr, 0, {fElc, hamilDerivKeElc, hamilElc, bField3d},
    {heatFluxLowerElc, heatFluxUpperElc})
  -- integrate all fields and store in dynvector
  runUpdater(fieldInt2d, tCurr, 0, {heatFluxLowerIon}, {totalHeatFluxLowerIon})
  runUpdater(fieldInt2d, tCurr, 0, {heatFluxUpperIon}, {totalHeatFluxUpperIon})
  runUpdater(fieldInt2d, tCurr, 0, {heatFluxLowerElc}, {totalHeatFluxLowerElc})
  runUpdater(fieldInt2d, tCurr, 0, {heatFluxUpperElc}, {totalHeatFluxUpperElc})
  -- store phi in a cell
  runUpdater(recordFieldInCellPhiCalc, 0, 0, {phi3d}, {fieldInCellPhi})
  -- compute total guiding center density by integrating over whole domain
  -- note: includes a volume factor that should be removed in post-processing
  runUpdater(fieldInt3d, tCurr, 0, {numDensityElc}, {totalElcCounter})
  runUpdater(fieldInt3d, tCurr, 0, {numDensityIon}, {totalIonCounter})
  -- compute relative error in guiding center density
  local gcError = math.abs(totalElcCounter:lastInsertedData()-totalIonCounter:lastInsertedData())/totalElcCounter:lastInsertedData()
  if gcError > 1e-8 then
    endExecution = true
  ...
  -- these are diagnostics called at the end of a time step in the rk3 method
  -- heat flux at edge
  runUpdater(heatFluxAtEdgeIonCalc, 0, 0, {fIon, hamilDerivKeIon, hamilIon, bField3d},
    {heatFluxLowerIon, heatFluxUpperIon})
  runUpdater(heatFluxAtEdgeElcCalc, 0, 0, {fElc, hamilDerivKeElc, hamilElc, bField3d},
    {heatFluxLowerElc, heatFluxUpperElc})
  -- integrate all fields and store in dynvector
  runUpdater(fieldInt2d, 0, 0, {heatFluxLowerIon}, {totalHeatFluxLowerIon})
  runUpdater(fieldInt2d, 0, 0, {heatFluxUpperIon}, {totalHeatFluxUpperIon})
  runUpdater(fieldInt2d, 0, 0, {heatFluxLowerElc}, {totalHeatFluxLowerElc})
  runUpdater(fieldInt2d, 0, 0, {heatFluxUpperElc}, {totalHeatFluxUpperElc})
  -- store phi in a cell
  runUpdater(recordFieldInCellPhiCalc, 0, 0, {phi3d}, {fieldInCellPhi})
  -- compute total guiding center density by integrating over whole domain
  runUpdater(fieldInt3d, 0, 0, {numDensityElc}, {totalElcCounter})
  runUpdater(fieldInt3d, 0, 0, {numDensityIon}, {totalIonCounter})
  -- compute relative error in guiding center density
  local gcError = math.abs(totalElcCounter:lastInsertedData()-totalIonCounter:lastInsertedData())/totalElcCounter:lastInsertedData()
  if gcError > 1e-8 then
    endExecution = true
