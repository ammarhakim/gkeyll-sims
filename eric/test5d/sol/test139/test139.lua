...
phaseDecomp = DecompRegionCalc5D.CartProd { cuts = {9, 18, 4, 1, 1} }
...
function fIonProfile(x,y,z,v,mu)
  ...
  return math.exp(-ionMass*(v-uParallel)^2/(2*0.9*ionTempProfile(x,y,z)*eV))*
...
if Lucee.IsRestarting then
  ...
  function initialDensityProfile(x,y,z)
    ...
    local effectiveSource = P_SOL/sourcePower*sourceDensityProfile(x,y,z)
    local nPeak = 4*math.sqrt(3)/3*math.sqrt(ionMass/(sourceTempProfile(x,y,z)*eV))*effectiveSource/2*(L_parallel/4)
