...
function elcTempProfile(x,y,z)
  ...
  return elcTemp--*math.exp(-r^2/r_s^2)
end

function fElcProfile(x,y,z,v,mu)
  return (2*math.pi*elcTempProfile(x,y,z)*eV/elcMass)^(-3/2)*
    math.exp(-elcMass*v^2/(2*elcTempProfile(x,y,z)*eV))*
...
initSourceIon = Updater.EvalOnNodes5D {
   ...
   -- function to use for initialization
   evaluate = function (x,y,z,v,mu,t)
		 return math.exp(-ionMass*v^2/(2*ionTemp*sourceTempProfile(x,y,z)*eV))*
      math.exp(-math.abs(mu)*bFieldProfile(x)/(ionTemp*sourceTempProfile(x,y,z)*eV))
	 end
}
...
scaleInitDistfIon = Updater.SOLInitializeDensity {
  ...
  evaluate = function(x,y,z,t)
    ...
    local nEdge = 1/20*n0
    if r <= a then
      return (n0-nEdge)*(1-r^2/a^2)^3 + nEdge
    else
      return nEdge
    end
  end
...
initSourceElc = Updater.EvalOnNodes5D {
   ...
   -- function to use for initialization
   evaluate = function(x,y,z,v,mu,t)
		 return math.exp(-elcMass*v^2/(2*elcTemp*sourceTempProfile(x,y,z)*eV))*
      math.exp(-math.abs(mu)*bFieldProfile(x)/(elcTemp*sourceTempProfile(x,y,z)*eV))
	 end
}
...
pbSlvrIon = Updater.PoissonBracketImp5D {
   ...
   zeroFluxDirectionsLower = {0,1,3,4},
   zeroFluxDirectionsUpper = {0,1,3,4},
   fluxType = "upwind",
}
...
function calcLenardBernsteinElc(tCurr, myDt, fInitial, fFinal)
  ...
  --Lucee.logInfo(string.format("-- calcLenardBernsteinElc Nbefore = %f --", avgElcDensity:lastInsertedData() ))
  ...
  --runUpdater(generalIntegralElcCalc, tCurr, myDt, {fFinal, oneFieldElc, bField3d}, {numDensityElc})
  --runUpdater(fieldInt3d, 0.0, 0.0, {numDensityElc}, {avgElcDensityDiagnostic})
  --Lucee.logInfo(string.format("-- calcLenardBernsteinElc Ndrag = %f --", avgElcDensityDiagnostic:lastInsertedData() ))
  ...
  --runUpdater(generalIntegralElcCalc, tCurr, myDt, {fFinal, oneFieldElc, bField3d}, {numDensityElc})
  --runUpdater(fieldInt3d, 0.0, 0.0, {numDensityElc}, {avgElcDensityDiagnostic})
  --Lucee.logInfo(string.format("-- calcLenardBernsteinElc Ndiff = %f --", avgElcDensityDiagnostic:lastInsertedData() ))
