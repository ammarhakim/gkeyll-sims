...
-- mpiexec -n 4 /Users/eshi/Research/gkeyllall/par-opt/gkeyll/gkeyll -i test112.lua -pc_type lu -pc_factor_mat_solver_package superlu_dist
filename = "test112"

-- phase-space decomposition (5d)
phaseDecomp = DecompRegionCalc5D.CartProd { cuts = {18, 18, 2, 1, 1} }
...
function rkLegendre2(tCurr, myDt)
  ...
  netDtSuggested = math.min(myDtSuggestedLBElcIon, netDtSuggested)
  ...
  netDtSuggested = math.min(myDtSuggestedLBIon, netDtSuggested)
  -- logic for ion-ion collisions
  if (myDt > myDtSuggestedLBIon) then
    local dtRatio = myDt/myDtSuggestedLBIonIon
    -- Set a cap on the max. stages allowed math.min(calcNumStages(dtRatio), 400)
    local numStages = calcNumStages(dtRatio)
    ...
    -- rest of stages
    for j = 2, numStages do
      calcLenardBernsteinIonIon(tCurr, myDt, fIonCollisions1, fIon, fNewIon)
  ...
  local stage1ElcFloorEnergy, stage1IonFloorEnergy = applyDensityFloor(tCurr, myDt, fElc, f1Ion)
