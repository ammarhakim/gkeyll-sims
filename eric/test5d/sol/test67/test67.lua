-- Input file for a 3X2V SOL Simulation of Helimak
-- 7-7-16: Based on test66.lua
-- mpiexec -n 4 /Users/eshi/Research/gkeyllall/par-opt/gkeyll/gkeyll -i test66.lua -pc_type lu -pc_factor_mat_solver_package superlu_dist -r 0
...
ionMass   = 39.659*Lucee.ProtonMass 
elcTemp   = 10 -- [eV]
ionTemp   = 0.1 -- [eV]
B0 = 0.1  -- [T]
n0 = 10^19 -- [1/m^3]
N = 4 -- field line turns
sourceTemp     = 10 -- [eV] -- temperature to base grid off of
sourceTempInit = 10 -- [eV] -- temperature to init source with
-- derived parameters
...
Z_UPPER = L_parallel
-- source parameters
sourceLambda    = 5*rho_s
sourceX         = R
sourceAmplitude = 0.1*n0*c_s/R -- [1/(m^3*sec)] 
-- total volume
...
function sourceTempProfile(x)
  return sourceTempInit*(0.95*math.exp(-(x-sourceX)^2/sourceLambda^2) + 0.05)
end

function sourceDensityProfile(x,y,z)
  return sourceAmplitude*(0.95*math.exp(-(x-sourceX)^2/sourceLambda^2) + 
      0.05)
end
...
function calcLenardBernsteinElc(tCurr, myDt, fInitial, fFinal)
  ...
  if (diffStatus == false or dragStatus == false) then
    --Lucee.logInfo(string.format("-- diffDtSuggested = %g --", diffDtSuggested))
    --Lucee.logInfo(string.format("-- dragDtSuggested = %g --", dragDtSuggested))
...
function calcDiagnostics(tCurr, myDt)
  ...
  runUpdater(bgPhiCalc, tCurr, myDt, {phi3d}, {bgPhi, varPhi})
  -- compute fluctuating potential
  fluctPhi:combine(1.0, phi3d, -1.0, bgPhi)
  ...
  -- compute background electron pressure using up to the past X samples
  runUpdater(bgPressureCalc, tCurr, myDt, {pressureElc}, {bgPressureElc, varPressureElc})
  -- compute fluctuating electron pressure
  fluctPressureElc:combine(1.0, pressureElc, -1.0, bgPressureElc)

  -- compute background electron temp using up to the past X samples
  runUpdater(bgTempCalc, tCurr, myDt, {temperatureElc}, {bgTempElc, varTempElc})
  -- compute fluctuating electron temp
  fluctTempElc:combine(1.0, temperatureElc, -1.0, bgTempElc)

  -- compute background electron density using up to the past X samples
  runUpdater(bgDensCalc, tCurr, myDt, {numDensityElc}, {bgDensityElc, varDensityElc})
  -- compute fluctuating electron density
  fluctDensityElc:combine(1.0, numDensityElc, -1.0, bgDensityElc)
...
function writeFields(frameNum, tCurr)
  ...
  bgPressureElc:write( string.format("bgPressureElc_%d.h5", frameNum), tCurr)
  fluctPressureElc:write( string.format("fluctPressureElc_%d.h5", frameNum), tCurr)
  varPressureElc:write( string.format("varPressureElc_%d.h5", frameNum), tCurr)

  numDensityElc:write( string.format("nElc_%d.h5", frameNum), tCurr)
  bgDensityElc:write( string.format("bgDensityElc_%d.h5", frameNum), tCurr)
  fluctDensityElc:write( string.format("fluctDensityElc_%d.h5", frameNum), tCurr)
  varDensityElc:write( string.format("varDensityElc_%d.h5", frameNum), tCurr)

  temperatureElc:write( string.format("tElc_%d.h5", frameNum), tCurr)
  bgTempElc:write( string.format("bgTempElc_%d.h5", frameNum), tCurr)
  fluctTempElc:write( string.format("fluctTempElc_%d.h5", frameNum), tCurr)
  varTempElc:write( string.format("varTempElc_%d.h5", frameNum), tCurr)

  bgPhi:write( string.format("bgPhi_%d.h5", frameNum), tCurr)
  varPhi:write( string.format("varPhi_%d.h5", frameNum), tCurr)
  fluctPhi:write( string.format("fluctPhi_%d.h5", frameNum), tCurr)
