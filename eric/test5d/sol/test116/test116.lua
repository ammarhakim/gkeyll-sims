...
-- mpiexec -n 4 /Users/eshi/Research/gkeyllall/par-opt/gkeyll/gkeyll -i test116.lua -pc_type lu -pc_factor_mat_solver_package superlu_dist
filename = "test116"
...
L_s = 0.5*rho_s
r_s = 20*rho_s
sourceAmplitude = 2*0.03*n0*c_s/R
-- total volume
...
initfElc = Updater.EvalOnNodes5D {
   ...
   evaluate = function (x,y,z,v,mu,t)
		 ...
		 local v_m = math.sqrt(elcTemp/2*eV*3/elcMass)
		 local mu_m = 2*elcTemp/2*eV/B0
...
initfEpsilonElc = Updater.EvalOnNodes5D {
   onGrid = grid_elc_5d,
   basis = basis_elc_5d,
   shareCommonNodes = false,
   -- function to use for initialization
...
runUpdater(initfEpsilonElc, 0.0, 0.0, {}, {fEpsilonElc})
fEpsilonElc:sync()
...
initfIon = Updater.EvalOnNodes5D {
   ...
   evaluate = function (x,y,z,v,mu,t)
		 ...
		 local v_m = math.sqrt(ionTemp*eV*3/ionMass)
		 local mu_m = 2*ionTemp*eV/B0
...
dragSlvrElcIon = Updater.LenardBernsteinDrag3D2VUpdater {
  onGrid = grid_elc_5d,
  basis5d = basis_elc_5d,
  basis3d = basis_3d,
  basis2d = basis_2d,
...
fElcCollisions0 = DataStruct.Field5D {
   onGrid = grid_elc_5d,
   numComponents = basis_elc_5d:numNodes(),
   ghost = {1, 1},
}
fElcCollisions1 = DataStruct.Field5D {
   onGrid = grid_elc_5d,
   numComponents = basis_elc_5d:numNodes(),
   ghost = {1, 1},
}
fElcCollisions2 = DataStruct.Field5D {
   onGrid = grid_elc_5d,
   numComponents = basis_elc_5d:numNodes(),
   ghost = {1, 1},
}
...
fIonCollisions0 = DataStruct.Field5D {
   onGrid = grid_ion_5d,
   numComponents = basis_ion_5d:numNodes(),
   ghost = {1, 1},
}
fIonCollisions1 = DataStruct.Field5D {
   onGrid = grid_ion_5d,
   numComponents = basis_ion_5d:numNodes(),
   ghost = {1, 1},
}
fIonCollisions2 = DataStruct.Field5D {
   onGrid = grid_ion_5d,
   numComponents = basis_ion_5d:numNodes(),
   ghost = {1, 1},
}
...
function applyDensityFloor(tCurr, myDt, distfElc, distfIon)
  ...
  return elcPowerAdded, ionPowerAdded
end

function calcPotential(nIon, nElc, phiOut)
  -- calculate average ion density
  local n_i0 = 4.0e17--avgIonDensity:lastInsertedData()/totalVol--2.325e17
  -- solve for potential
...
function calcLenardBernsteinElcElc(tCurr, myDt, fInitial, fFinal)
  ...
  -- accumulate drag term times 1 to distribution function
  fFinal:copy(fElcCollisions)
  
  runUpdater(diffSlvrElc, tCurr, myDt, {fInitial, temperatureElc, temperatureElc,
    temperatureElc, bField3d, numDensityElc}, {fElcCollisions})
...
function calcLenardBernsteinElcIon(tCurr, myDt, fInitialElc, fInitialIon, fFinal)
  ...
  -- compute drag term
  local dragStatus, dragDtSuggested = runUpdater(dragSlvrElcIon, tCurr, myDt, {fInitialElc, mom1dir3Ion,
    numDensityIon, temperatureElc, numDensityElc}, {fElcCollisions})
  -- compute energy at each node in the cell for drag term only
  runUpdater(elcEnergyAtCellCalc, tCurr, myDt, {fElcCollisions, jacobianFieldElc, hamilKeElc}, {elcEnergyAtNodeDrag})
  -- copy drag term to distribution function (no dt factors)
  fFinal:copy(fElcCollisions)
  
  runUpdater(diffSlvrElcIon, tCurr, myDt, {fInitialElc, temperatureElc, temperatureElc, temperatureElc, bField3d,
    numDensityElc}, {fElcCollisions})
  ...
  local diffStatus, diffDtSuggested = runUpdater(lenardBernsteinScaleElcIonUpdater,
    ...
    elcEnergyAtNodeDiff, temperatureElc, temperatureElc, temperatureElc, bField3d, numDensityElc}, {fFinal})
...
function rkLegendre2(tCurr, myDt)
  -- non-collisional terms
  -- compute f1 = f + dt*deltaF, so super-stepped terms will need to subtract original f before being added
  ...
  --local netStatus = (myStatusElc == false or myStatusIon == false)
  --local netDtSuggested = math.min(myDtSuggestedElc, myDtSuggestedIon)
  ...
  -- collisional terms
  -- calculate fElcCollisions0 (this will not change until next rkLegendre2 call)
  local myStatusLBElc, myDtSuggestedLBElc = calcLenardBernsteinElcElc(tCurr, myDt, fElc, fElcCollisions0)
  local netDtSuggested = math.max(myDtSuggestedLBElc, myDt) -- so that we don't get a ratio less than one

  -- logic for electron-electron collisions
  if (myDt > myDtSuggestedLBElc) then
    local dtRatio = myDt/myDtSuggestedLBElc
    -- Set a cap on the max. stages allowed math.min(calcNumStages(dtRatio), 400)
    local numStages = calcNumStages(dtRatio)
    Lucee.logInfo(string.format("ElcElc using dtRatio = %g, numStage = %g", dtRatio, numStages))

    -- stage 1
    fElcCollisions2:copy(fElc)
    fElcCollisions1:combine(1.0, fElc, mubar(numStages,1)*myDt, fElcCollisions0)
    --applyElcPositivity(tCurr, myDt, fElcCollisions1)

    -- rest of stages
    for j = 2, numStages do
      calcLenardBernsteinElcElc(tCurr, myDt, fElcCollisions1, fNewElc)

      -- endExecution can be set to true if a negative temperature is computed in the LBO
      if endExecution == true then
        return false, 0.0
      end

      fElcCollisions:combine(mu(j), fElcCollisions1, nu(j), fElcCollisions2, 1-mu(j)-nu(j), fElc,
        mubar(numStages,j)*myDt, fNewElc, gbar(numStages,j)*myDt, fElcCollisions0)
      -- reset fields for next stage
      fElcCollisions2:copy(fElcCollisions1)

      fElcCollisions1:copy(fElcCollisions)
    end
    fElcCollisions:accumulate(-1.0, fElc)
    f1Elc:accumulate(1.0, fElcCollisions)
  else
    f1Elc:accumulate(myDt, fElcCollisions0)
  end

  -- First compute the increment deltaF (not an actual time step)
  local myStatusLBElcIon, myDtSuggestedLBElcIon = calcLenardBernsteinElcIon(tCurr, myDt, fElc, fIon, fElcCollisions0)
  netDtSuggested = math.max(myDtSuggestedLBElcIon, netDtSuggested)
  -- logic for electron-ion collisions
  if (myDt > myDtSuggestedLBElc) then
    local dtRatio = myDt/myDtSuggestedLBElcIon
    -- Set a cap on the max. stages allowed math.min(calcNumStages(dtRatio), 400)
    local numStages = calcNumStages(dtRatio)
    Lucee.logInfo(string.format("ElcIon using dtRatio = %g, numStage = %g", dtRatio, numStages))
    ...
    -- rest of stages
    for j = 2, numStages do
      calcLenardBernsteinElcIon(tCurr, myDt, fElcCollisions1, fIon, fNewElc)
  ...
  -- First compute the increment deltaF (not an actual time step)
  local myStatusLBIon, myDtSuggestedLBIon = calcLenardBernsteinIonIon(tCurr, myDt, fIon, fIonCollisions0)
  netDtSuggested = math.max(myDtSuggestedLBIon, netDtSuggested)
  -- logic for ion-ion collisions
  if (myDt > myDtSuggestedLBIon) then
    local dtRatio = myDt/myDtSuggestedLBIon
    -- Set a cap on the max. stages allowed math.min(calcNumStages(dtRatio), 400)
    local numStages = calcNumStages(dtRatio)
    Lucee.logInfo(string.format("IonIon using dtRatio = %g, numStage = %g", dtRatio, numStages))

    -- stage 1
    fIonCollisions2:copy(fIon)
    fIonCollisions1:combine(1.0, fIon, mubar(numStages,1)*myDt, fIonCollisions0)
    --applyIonPositivity(tCurr, myDt, fIonCollisions1)

    -- rest of stages
    for j = 2, numStages do
      calcLenardBernsteinIonIon(tCurr, myDt, fIonCollisions1, fNewIon)
      ...
      fIonCollisions:combine(mu(j), fIonCollisions1, nu(j), fIonCollisions2, 1-mu(j)-nu(j), fIon,
        mubar(numStages,j)*myDt, fNewIon, gbar(numStages,j)*myDt, fIonCollisions0)
      -- reset fields for next stage
      fIonCollisions2:copy(fIonCollisions1)

      fIonCollisions1:copy(fIonCollisions)
    end
    fIonCollisions:accumulate(-1.0, fIon)
    --fIon:copy(fIonCollisions)
    f1Ion:accumulate(1.0, fIonCollisions)
  else
    f1Ion:accumulate(myDt, fIonCollisions0)
  end
  ...
  -- copy f^{n+1} to f
  fElc:accumulate(1.0, f1Elc)
  fIon:accumulate(1.0, f1Ion)

  --local stage1ElcFloorEnergy, stage1IonFloorEnergy = applyDensityFloor(tCurr, myDt, fElc, fIon)
  -- apply positivity to electrons
  --local stage1ElcEnergy = applyElcPositivity(tCurr, myDt, fElc)
  -- apply positivity to ions
  --local stage1IonEnergy = applyIonPositivity(tCurr, myDt, fIon)

  --Lucee.logInfo(string.format("Total elc energy added from positivity = %g", stage1ElcEnergy))
  --Lucee.logInfo(string.format("Total elc energy added from floor = %g", stage1ElcFloorEnergy))
  --Lucee.logInfo(string.format("Total ion energy added from positivity = %g", stage1IonEnergy))
  --Lucee.logInfo(string.format("Total ion energy added from floor = %g", stage1IonFloorEnergy))
  ...
  --applyBcToDistF(tCurr, myDt, fElc, fIon, phi3d)
  fElc:sync()
  fIon:sync()
...
function calcDiagnostics(tCurr, myDt)
  ...
  kineticEnergyElc:clear(0.0)
  kineticEnergyElc:accumulate(1/6, kineticEnergyStage1Elc)
  kineticEnergyElc:accumulate(2/3, kineticEnergyStage2Elc)
  kineticEnergyElc:accumulate(1/6, kineticEnergyStage3Elc)
end
