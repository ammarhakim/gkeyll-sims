...
-- mpiexec -n 4 /Users/eshi/Research/gkeyllall/par-opt/gkeyll/gkeyll -i test73.lua -pc_type lu -pc_factor_mat_solver_package superlu_dist
...
scaleInitDistfIon = Updater.SOLInitializeDensity {
  ...
  scaleFactor = 2*math.pi/ionMass,
  evaluate = function(x,y,z,t)
    return 0.6214*n0*initBackgroundProfile(x,y,z)
  end
}
...
fieldSmoother = Updater.FemPoisson2D {
   ...
   -- use neumann bc in all directions
...
avgElcDensityDiagnostic = DataStruct.DynVector { numComponents = 1, }
...
function calcPotential(phiSLower, phiSUpper,nIon, nElc, phiOut)
...
function rk3(tCurr, myDt)
  ...
  -- solve for potential
  local potentialStatus = calcPotential(phiSLower, phiSUpper, numDensityIon, numDensityElc, phi3d)
  ...
  runUpdater(generalIntegralIonCalc, tCurr, myDt, {f1Ion, oneFieldIon, bField3d}, {numDensityIon})
  -- solve for potential
  potentialStatus = calcPotential(phiSLower, phiSUpper, numDensityIon, numDensityElc, phi3d)
  if (potentialStatus == false) then
    endExecution = true
    calcContinuousDiagnostics(tCurr, myDt)
    return false, math.min(myDtSuggestedElc, myDtSuggestedIon)
...
if Lucee.IsRestarting then
  ...
  -- solve for potential
  calcPotential(phiSLower, phiSUpper, numDensityIon, numDensityElc, phi3d)
  -- calculate new hamiltonians
  calcHamiltonianElc(phi3d, hamilElc)
  calcHamiltonianIon(phi3d, hamilIon)
else
