...
-- mpiexec -n 4 /Users/eshi/Research/gkeyllall/par-opt/gkeyll/gkeyll -i test41.lua -pc_type lu -pc_factor_mat_solver_package superlu_dist -r 0
...
function applyPositivity(tCurr, myDt, distfElc, distfIon)
  ...
  Lucee.logInfo(string.format("-- elc number before positivity = %g --", avgElcDensity:lastInsertedData() ))
  
  runUpdater(numDensityElcCalc, tCurr, myDt, {distfIon, bField3d}, {numDensityIon})
  numDensityIon:scale(2*math.pi/ionMass)
  runUpdater(fieldInt3d, tCurr, myDt, {numDensityIon}, {avgIonDensity})
  Lucee.logInfo(string.format("-- ion number before positivity = %g --", avgIonDensity:lastInsertedData() ))
  ...
  numDensityElc:scale(2*math.pi/elcMass)
  runUpdater(fieldInt3d, tCurr, myDt, {numDensityElc}, {avgElcDensity})
  Lucee.logInfo(string.format("-- elc number after positivity = %g --", avgElcDensity:lastInsertedData() ))
  
  runUpdater(numDensityElcCalc, tCurr, myDt, {distfIon, bField3d}, {numDensityIon})
  numDensityIon:scale(2*math.pi/ionMass)
  runUpdater(fieldInt3d, tCurr, myDt, {numDensityIon}, {avgIonDensity})
  Lucee.logInfo(string.format("-- ion number after positivity = %g --", avgIonDensity:lastInsertedData() ))
...
function computePotential(nIon, nElc, phiOut)
...
function rk3(tCurr, myDt)
  ...
  numDensityIon:scale(2*math.pi/ionMass)
  -- solve for potential
  local potentialStatus = computePotential(numDensityIon, numDensityElc, phi3d)
  ...
  -- calculate density after rk1 step
  runUpdater(numDensityElcCalc, tCurr, myDt, {f1Elc, bField3d}, {numDensityElc})
  numDensityElc:scale(2*math.pi/elcMass)
  runUpdater(numDensityIonCalc, tCurr, myDt, {f1Ion, bField3d}, {numDensityIon})
  numDensityIon:scale(2*math.pi/ionMass)
  ...
  Lucee.logInfo(string.format("-- elc number rk1 timestep = %g --", totalElcCounter:lastInsertedData() ))
  Lucee.logInfo(string.format("-- ion number rk1 timestep = %g --", totalIonCounter:lastInsertedData() ))
  ...
  numDensityIon:scale(2*math.pi/ionMass)
  -- solve for potential
  potentialStatus = computePotential(numDensityIon, numDensityElc, phi3d)
  if (potentialStatus == false) then
    endExecution = true
    fElc:copy(f1Elc)
    fIon:copy(f1Ion)
...
Lucee.logInfo(string.format("-- elc number between timestep = %g --", totalElcCounter:lastInsertedData() ))
Lucee.logInfo(string.format("-- ion number between timestep = %g --", totalIonCounter:lastInsertedData() ))
...
if Lucee.IsRestarting then
  ...
  numDensityIon:scale(2*math.pi/ionMass)
  -- solve for potential
  computePotential(numDensityIon, numDensityElc, phi3d)
  -- calculate new hamiltonians
  computeHamiltonianElc(phi3d, hamilElc)
  computeHamiltonianIon(phi3d, hamilIon)
  -- apply boundary conditions to distribution functions
