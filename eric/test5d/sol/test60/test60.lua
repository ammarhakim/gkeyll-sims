...
-- mpiexec -n 1 /Users/eshi/Research/gkeyllall/par-opt/gkeyll/gkeyll -i test60.lua -pc_type lu -pc_factor_mat_solver_package superlu_dist
...
mom1dir3ElcMaxwellianNumericalNextStage = DataStruct.Field3D {
   onGrid = grid_3d,
   numComponents = basis_3d:numNodes(),
   ghost = {1, 1},
}
...
temperatureElcMaxwellianNumericalNextStage = DataStruct.Field3D {
   onGrid = grid_3d,
   numComponents = basis_3d:numNodes(),
   ghost = {1, 1},
}
...
newtonMethodSlope = DataStruct.Field3D {
   onGrid = grid_3d,
   numComponents = basis_3d:numNodes(),
   ghost = {1, 1},
}
backtrackLambdaCurrent = DataStruct.Field3D {
   onGrid = grid_3d,
   numComponents = basis_3d:numNodes(),
   ghost = {1, 1},
}
backtrackLambdaPrev = DataStruct.Field3D {
   onGrid = grid_3d,
   numComponents = basis_3d:numNodes(),
   ghost = {1, 1},
}
...
fieldProductCalc = Updater.FieldArithmeticUpdater3D {
  onGrid = grid_3d,
  basis = basis_3d,
  evaluate = function(fld1,fld2,t)
    return fld1*fld2
  end,
}
...
function calcCollisionsElc(tCurr, myDt, fInitial, fFinal)
  ...
  -- Initia guess for Newton's method
  ...
  while convergenceStatus == false and iterCount < maxIter do
    ...
    -- compute a full step (lambda = 1) of the newton's method
    convergenceStatus = runUpdater(maxwellianNewtonSearch, 0.0, 0.0, {mom1dir3Elc, temperatureElc,
      ...
      {mom1dir3ElcMaxwellianOutput, temperatureElcMaxwellianOutput, newtonMethodSlope})

    -- line search - backtracking algorithm
    backtrackLambdaCurrent:clear(1.0)

    -- compute maxwellian at new location
    temperatureElcMaxwellianIncrement:clear(0.0)
    mom1dir3ElcMaxwellianIncrement:clear(0.0)
    runUpdater(fieldProductCalc, tCurr, myDt, {backtrackLambdaCurrent, mom1dir3ElcMaxwellianOutput},
      {mom1dir3ElcMaxwellianIncrement})
    runUpdater(fieldProductCalc, tCurr, myDt, {backtrackLambdaCurrent, temperatureElcMaxwellianOutput},
      {temperatureElcMaxwellianIncrement})
    mom1dir3ElcMaxwellianInput:accumulate(1.0, mom1dir3ElcMaxwellianIncrement)
    temperatureElcMaxwellianInput:accumulate(1.0, temperatureElcMaxwellianIncrement)
    -- evaluate u and T at new location
    maxwellianValid = runUpdater(maxwellianEval, 0.0, 0.0, {numDensityElc, mom1dir3ElcMaxwellianInput,
      temperatureElcMaxwellianInput, bField3d}, {fElcMaxwellian})
    if maxwellianValid == false then
      Lucee.logInfo (string.format("-- Maxwellian invalid"))
    ...
    -- compute parameters of maxwellian (it should already have correct number density)
    runUpdater(generalIntegralElcCalc, 0.0, 0.0, {fElcMaxwellian, hamilDerivKeElc, bField3d},
      {mom1dir3ElcMaxwellianNumericalNextStage})
    runUpdater(generalIntegralElcCalc, 0.0, 0.0, {fElcMaxwellian, hamilKeElc, bField3d},
      {kineticEnergyElc})
    -- compute elc temperature
    runUpdater(temperatureElcCalc, 0.0, 0.0, {numDensityElc,kineticEnergyElc,mom1dir3ElcMaxwellianNumericalNextStage},
      {temperatureElcMaxwellianNumericalNextStage})

    -- compute f = 1/2 F.dot(F) at each location
    runUpdater(fieldProductCalc, tCurr, myDt,
      {mom1dir3ElcMaxwellianNumericalNextStage, mom1dir3ElcMaxwellianNumericalNextStage}, {})
    runUpdater(fieldProductCalc, tCurr, myDt,
      {temperatureMaxwellianNumericalNextStage, temperatureMaxwellianNumericalNextStage}, {})
...
function rk3(tCurr, myDt)
  ...
  if (netStatus == true or endExecution == true) then
  ...
  if (potentialStatus == false or endExecution == true) then
    endExecution = true
  ...
  netStatus = (myStatusElc == false or myStatusIon == false)
  netDtSuggested = math.min(myDtSuggestedElc, myDtSuggestedIon)
