...
-- mpiexec -n 1 /Users/eshi/Research/gkeyllall/par-opt/gkeyll/gkeyll -i positivity.lua -pc_type lu -pc_factor_mat_solver_package superlu_dist
...
initSourceIon = Updater.EvalOnNodes5D {
   ...
   -- function to use for initialization
   evaluate = function (x,y,z,v,mu,t)
		 return (sourceAmplitude_UH*math.exp(-(x-sourceX_UH)^2/sourceLambda_UH^2) + 
      sourceAmplitude_EC*math.exp(-(x-sourceX_EC)^2/sourceLambda_EC^2))* 
...
logicalSheathElcCalc = Updater.LogicalSheath5D {
  basis5d = basis_elc_5d,
  basis3d = basis_3d,
  basis2d = basis_2d,
  onGrid = grid_elc_5d,
...
logicalSheathIonCalc = Updater.LogicalSheath5D {
  basis5d = basis_ion_5d,
  basis3d = basis_3d,
  basis2d = basis_2d,
  onGrid = grid_ion_5d,
...
positivityDragIonUpdater = Updater.SOLPositivityDragUpdater {
  onGrid = grid_ion_5d,
  basis5d = basis_ion_5d,
  basis3d = basis_3d,
  basis2d = basis_2d,
...
positivityDragElcUpdater = Updater.SOLPositivityDragUpdater {
  onGrid = grid_elc_5d,
  basis5d = basis_elc_5d,
  basis3d = basis_3d,
  basis2d = basis_2d,
...
positivityScaleIonUpdater = Updater.SOLPositivityScaleUpdater {
  onGrid = grid_ion_5d,
  basis5d = basis_ion_5d,
  basis3d = basis_3d,
  basis2d = basis_2d,
...
positivityScaleElcUpdater = Updater.SOLPositivityScaleUpdater {
  onGrid = grid_elc_5d,
  basis5d = basis_elc_5d,
  basis3d = basis_3d,
  basis2d = basis_2d,
...
-- diagnostic to calculate total energy
totalElcEnergyCalc = Updater.SOLTotalEnergyCalc {
  onGrid = grid_elc_5d,
  basis = basis_elc_5d,
  scaleFactor = 2*math.pi/(elcMass)^3,
}
totalElcEnergyVec = DataStruct.DynVector { numComponents = 1, }

totalIonEnergyCalc = Updater.SOLTotalEnergyCalc {
  onGrid = grid_ion_5d,
  basis = basis_ion_5d,
  scaleFactor = 2*math.pi/(ionMass)^3,
}
totalIonEnergyVec = DataStruct.DynVector { numComponents = 1, }

ionEnergyAtNodeCalc = Updater.SOLEnergyAtNodeCalc {
  onGrid = grid_ion_5d,
  basis5d = basis_ion_5d,
  basis3d = basis_3d,
  basis2d = basis_2d,
...
ionEnergyAtNodeBeforePositivity = DataStruct.Field3D {
   onGrid = grid_3d,
   numComponents = basis_3d:numNodes(),
   ghost = {1, 1},
}
ionEnergyAtNodeAfterPositivity = DataStruct.Field3D {
   onGrid = grid_3d,
   numComponents = basis_3d:numNodes(),
   ghost = {1, 1},
}
ionEnergyAtNodeAfterMaxDrag = DataStruct.Field3D {
   onGrid = grid_3d,
   numComponents = basis_3d:numNodes(),
   ghost = {1, 1},
}

totalIonTempVec = DataStruct.DynVector { numComponents = 1, }
totalElcTempVec = DataStruct.DynVector { numComponents = 1, }
...
function applyPositivity(distfElc, distfIon)
  ...
  local elcStatus = runUpdater(localPositivityElcUpdater, 0.0, 0.0, {}, {distfElc})

  -- calculate initial ion energy
  runUpdater(totalIonEnergyCalc, 0.0, 0.0, {distfIon, jacobianFieldIon, hamilParaIon}, {totalIonEnergyVec})
  runUpdater(numDensityIonCalc, 0.0, 0.0, {distfIon, bField3d}, {numDensityIon})
  ...
  Lucee.logInfo (string.format("-- Ion energy before positivity = %.10g", totalIonEnergyVec:lastInsertedData()))
  Lucee.logInfo (string.format("-- Ion number before positivity = %.10g", avgIonDensity:lastInsertedData()))
  

  -- compute ion parallel energy at nodes before positivity is applied
  runUpdater(ionEnergyAtNodeCalc, 0.0, 0.0, {distfIon, hamilParaIon}, {ionEnergyAtNodeBeforePositivity})
  -- apply positivity to ions
  local ionStatus = runUpdater(localPositivityIonUpdater, 0.0, 0.0, {}, {distfIon})
  runUpdater(totalIonEnergyCalc, 0.25, 0.0, {distfIon, jacobianFieldIon, hamilParaIon}, {totalIonEnergyVec})
  runUpdater(numDensityIonCalc, 0.0, 0.0, {distfIon, bField3d}, {numDensityIon})
  numDensityIon:scale(2*math.pi/ionMass)
  -- store ion density temporarily in avgIonDensity structure
  runUpdater(fieldInt3d, 0.25, 0.0, {numDensityIon}, {avgIonDensity})
  Lucee.logInfo (string.format("-- Ion energy after pos step = %.10g", totalIonEnergyVec:lastInsertedData()))
  Lucee.logInfo (string.format("-- Ion number after pos step = %.10g", avgIonDensity:lastInsertedData()))
  
  numDensityDelta:copy(numDensityIon)
  numDensityIon:write( string.format("numDensityIonPos_%d.h5", 0), 0)

  -- compute ion parallel energy at nodes after positivity is applied
  runUpdater(ionEnergyAtNodeCalc, 0.0, 0.0, {distfIon, hamilParaIon}, {ionEnergyAtNodeAfterPositivity})

  -- need to sync ghost cells for drag update
  distfIon:sync()
  runUpdater(positivityDragIonUpdater, 0.0, 0.0, {distfIon, ionEnergyAtNodeAfterPositivity,
    ionEnergyAtNodeBeforePositivity}, {fIonPositivityDrag})
  -- compute ion parallel energy at nodes after maximum drag step
  runUpdater(ionEnergyAtNodeCalc, 0.0, 0.0, {fIonPositivityDrag, hamilParaIon},
    {ionEnergyAtNodeAfterMaxDrag})

  runUpdater(totalIonEnergyCalc, 0.5, 0.0, {fIonPositivityDrag, jacobianFieldIon, hamilParaIon}, {totalIonEnergyVec})
  runUpdater(numDensityIonCalc, 0.0, 0.0, {fIonPositivityDrag, bField3d}, {numDensityIon})
  numDensityIon:scale(2*math.pi/ionMass)
  -- store ion density temporarily in avgIonDensity structure
  runUpdater(fieldInt3d, 0.5, 0.0, {numDensityIon}, {avgIonDensity})
  Lucee.logInfo (string.format("-- Ion energy after drag step = %.10g", totalIonEnergyVec:lastInsertedData()))
  Lucee.logInfo (string.format("-- Ion number after drag step = %.10g", avgIonDensity:lastInsertedData()))
  
  numDensityDelta:accumulate(-1.0, numDensityIon)
  numDensityDelta:write( string.format("numDensityDelta_%d.h5", 0), 0)
  numDensityIon:write( string.format("numDensityIonDrag_%d.h5", 0), 0)
  ...
  -- Use {ionEnergyAtNodeBeforePositivity, ionEnergyAtNodeAfterPositivity, ionEnergyAtNodeAfterMaxDrag}
  -- to get correct energy at each node
  runUpdater(positivityScaleIonUpdater, 0.0, 0.0, {fIonPositivityDelta, ionEnergyAtNodeBeforePositivity,
    ionEnergyAtNodeAfterPositivity, ionEnergyAtNodeAfterMaxDrag}, {distfIon})
  
  distfIon:sync()
  -- recalculate total energy now
  runUpdater(totalIonEnergyCalc, 0.75, 0.0, {distfIon, jacobianFieldIon, hamilParaIon}, {totalIonEnergyVec})
  runUpdater(numDensityIonCalc, 0.0, 0.0, {distfIon, bField3d}, {numDensityIon})
  numDensityIon:scale(2*math.pi/ionMass)
  -- store ion density temporarily in avgIonDensity structure
  runUpdater(fieldInt3d, 0.75, 0.0, {numDensityIon}, {avgIonDensity})
  Lucee.logInfo (string.format("-- Ion energy after scale step = %.10g", totalIonEnergyVec:lastInsertedData()))
  Lucee.logInfo (string.format("-- Ion number after scale step = %.10g", avgIonDensity:lastInsertedData()))

  if (elcStatus == false or ionStatus == false) then
    Lucee.logInfo(string.format("-- ending execution because entire (v,mu) cell was negative --"))
    endExecution = true
  end
end
...
function applyBcToDistF(tCurr, myDt, fElc, fIon)
  ...
  -- apply zero inflow boundary conditions in z to electrons
  runUpdater(bcLowerZElc, tCurr, myDt, {}, {fElc})
  runUpdater(bcUpperZElc, tCurr, myDt, {}, {fElc})
  -- calculate first vPar moment of the ion distribution function (don't integrate ghosts)
  ...
  sheathStatus = runUpdater(logicalSheathElcCalc, tCurr, myDt, {edgeMomentsIon, hamilDerivKeElc},
    {fElc, phiSLower, phiSUpper})
  if sheathStatus == false then
    Lucee.logInfo(string.format("-- elc logical sheath failed --"))
    return sheathStatus
  end
  -- calculate first vPar moment of the elc distribution function (don't integrate ghosts)
  runUpdater(edgeMomentCalcElc, tCurr, myDt, {fElc, hamilDerivKeElc}, {edgeMomentsElc})
  -- apply logical sheath boundary conditions to ion distribution function
  sheathStatus = runUpdater(logicalSheathIonCalc, tCurr, myDt, {edgeMomentsElc, hamilDerivKeIon},
    {fIon, phiSLower, phiSUpper})
  if sheathStatus == false then
    Lucee.logInfo(string.format("-- ion logical sheath failed --"))
    return sheathStatus
  end
...
function rk3(tCurr, myDt)
  ...
  return numberConservationStatus, math.min(myDtSuggestedElc, myDtSuggestedIon)
end
...
function calcDiagnostics(tCurr, myDt)
  ...
  --runUpdater(edgeMomentCalcElc, tCurr, myDt, {fElc, hamilDerivKeElc}, {edgeMomentsElc})
  --edgeMomentsDelta:combine(1/elcMass, edgeMomentsElc, -1/ionMass, edgeMomentsIon)
...
function calcSourceDiagnostics(tCurr, myDt)
  ...
  -- compute electron moments
  runUpdater(mom1VParaElcCalc, tCurr, myDt, {elcSource, bField3d}, {mom1dir3Elc})
  mom1dir3Elc:scale(2*math.pi/elcMass)
  runUpdater(mom2VParaElcCalc, tCurr, myDt, {elcSource, bField3d}, {mom2dir3Elc})
  mom2dir3Elc:scale(2*math.pi/elcMass)
  runUpdater(mom1MuElcCalc, tCurr, myDt, {elcSource, bField3d}, {mom1dir4Elc})
  mom1dir4Elc:scale(2*math.pi/elcMass)
  -- compute elctron temperature
  runUpdater(temperatureElcCalc, tCurr, myDt, {numDensityElc,mom1dir3Elc,mom2dir3Elc,mom1dir4Elc,bField3d},
    {temperatureElc})
  ...
  -- compute ion moments
  runUpdater(mom1VParaIonCalc, tCurr, myDt, {ionSource, bField3d}, {mom1dir3Ion})
  mom1dir3Ion:scale(2*math.pi/ionMass)
  runUpdater(mom2VParaIonCalc, tCurr, myDt, {ionSource, bField3d}, {mom2dir3Ion})
  mom2dir3Ion:scale(2*math.pi/ionMass)
  runUpdater(mom1MuIonCalc, tCurr, myDt, {ionSource, bField3d}, {mom1dir4Ion})
  mom1dir4Ion:scale(2*math.pi/ionMass)
  -- compute ion temperature
  runUpdater(temperatureIonCalc, tCurr, myDt, {numDensityIon,mom1dir3Ion,mom2dir3Ion,mom1dir4Ion,bField3d},
    {temperatureIon})
  ...
  runUpdater(fieldInt3d, tCurr, myDt, {temperatureElc}, {totalElcTempVec})
  print(string.format( 'Average source elc temp = %g eV',totalElcTempVec:lastInsertedData()/volume ))
  runUpdater(fieldInt3d, tCurr, myDt, {temperatureIon}, {totalIonTempVec})
  print(string.format( 'Average source ion temp = %g eV',totalIonTempVec:lastInsertedData()/volume ))
end
...
function calcContinuousDiagnostics(tCurr, myDt)
  ...
  -- compute relative error in guiding center density
  local gcError = math.abs(totalElcCounter:lastInsertedData()-totalIonCounter:lastInsertedData())/totalElcCounter:lastInsertedData()
  if gcError > 1e-8 then
    endExecution = true
    Lucee.logInfo (string.format("*** Ending execution because guiding center error is %g", gcError))
    return false
  end
...
applyPositivity(elcSource, ionSource)
calcSourceDiagnostics(0.0, 0.0)
...
if Lucee.IsRestarting then
  ...
  fIon:write( string.format("fIonB_%d.h5", 0), 0)
  -- Make sure distribution functions are positive
  --applyPositivity(fElc, fIon)
  fIon:write( string.format("fIonA_%d.h5", 0), 0)
