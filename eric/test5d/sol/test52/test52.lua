...
-- mpiexec -n 4 /Users/eshi/Research/gkeyllall/par-opt/gkeyll/gkeyll -i test52.lua -pc_type lu -pc_factor_mat_solver_package superlu_dist -r 2218
...
--fieldInCellPhi_alt = DataStruct.DynVector { numComponents = basis_3d:numNodes(), }
...
function calcLenardBernsteinElc(tCurr, myDt, fInitial, fInitIon, fFinal)
  ...
  runUpdater(numDensityElcCalc, tCurr, myDt, {fFinal, bField3d}, {numDensityElc})
  numDensityElc:scale(2*math.pi/elcMass)
  -- store elc density temporarily in avgElcDensity structure
  runUpdater(fieldInt3d, tCurr, myDt, {numDensityElc}, {avgElcDensity})
  local elcNumberStart = avgElcDensity:lastInsertedData()
  ...
  --runUpdater(numDensityElcCalc, tCurr, myDt, {fInitial, bField3d}, {numDensityElc})
  --numDensityElc:scale(2*math.pi/elcMass)
  --runUpdater(fieldInt3d, 0.0, 0.0, {numDensityElc}, {avgElcDensity})
  --runUpdater(recordFieldInCellPhiCalc, tCurr, myDt, {numDensityElc}, {fieldInCellPhi_alt})
  --Lucee.logInfo(string.format("-- Method 1 = %g --", avgElcDensity:lastInsertedData()))
  
  runUpdater(generalIntegralElcTestCalc, tCurr, myDt, {fInitial, oneFieldElc, bField3d}, {numDensityElc})
  runUpdater(fieldInt3d, 0.0, 0.0, {numDensityElc}, {avgElcDensity})
  --Lucee.logInfo(string.format("-- Method 2 = %g --", avgElcDensity:lastInsertedData()))
  
  -- compute electron temperature (in joules)
  runUpdater(recordFieldInCellPhiCalc, tCurr, myDt, {numDensityElc}, {fieldInCellPhi})
  local negativeTemperature = runUpdater(temperatureElcCalc, tCurr, myDt, {numDensityElc,kineticEnergyElc,mom1dir3Elc},
    {temperatureElc})
  ...
  Lucee.logInfo(string.format("-- Before = %g, After = %g --", elcNumberStart, elcNumberEnd))
...
function rk3(tCurr, myDt)
  ...
  local myStatusLBElc, myDtSuggestedLBElc = calcLenardBernsteinElc(tCurr, myDt, fElc, fIon, f1Elc)
  ...
  myStatusLBElc, myDtSuggestedLBElc = calcLenardBernsteinElc(tCurr, myDt, f1Elc, f1Ion, fNewElc)
...
function writeFields(frameNum, tCurr)
  ...
  --fieldInCellPhi_alt:write( string.format("phiInCellAlt_%d.h5", frameNum), tCurr)
end
