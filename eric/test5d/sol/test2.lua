...
-- 3-23-16: modified to test sheath bc
-- 4-1-2015: appears outdated
...
function computePotential(phiSLower, phiSUpper, nIon, nElc, phiOut)
  ...
  runUpdater(fieldSolver, 0.0, 0.0, {numDensityDelta, dirichletBoundaryField}, {phiOut})
end
...
function applyBcToDistF(tCurr, myDt, fElc, fIon)
  ...
  status = runUpdater(logicalSheathCalc, tCurr, myDt, {edgeMomentsIon}, {fElc, phiSLower, phiSUpper})
  if (status == false) then
    print('sheath bc failed')
  end
...
function rk3(tCurr, myDt)
  ...
  applyBcToDistF(tCurr, myDt, f1Elc, f1Ion)
  ...
  -- RK stage 2
  local myStatusElc, myDtSuggestedElc = runUpdater(pbSlvrElc, tCurr, myDt, {f1Elc, hamilElc}, {fNewElc})
  local myStatusIon, myDtSuggestedIon = runUpdater(pbSlvrIon, tCurr, myDt, {f1Ion, hamilIon}, {fNewIon})
  if (myStatusElc == false or myStatusIon == false) then
    return false, math.min(myDtSuggestedElc, myDtSuggestedIon)
  end
  -- add on particle source
