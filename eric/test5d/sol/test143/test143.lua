...
function bFieldProfile(x)
  return B0--B_axis*R0/(R0+a0) + (x - R0 - a0 )/bFieldReduction )
end
...
function sourceTempProfile(x,y,z)
  ...
  if (x < xSource) then
    ...
    return (elcSourceTempMax-elcSourceTempMin)*math.exp(-(x-xSource)/lambdaSource) + elcSourceTempMin
  end
end
...
initbStarYElc = Updater.EvalOnNodes5D {
  ...
  shareCommonNodes = false,
  evaluate = function (x,y,z,vPara,mu,t)
    return 0-- -elcMass*vPara/(elcCharge*x)
  end
}
...
initbStarYIon = Updater.EvalOnNodes5D {
  ...
  shareCommonNodes = false,
  evaluate = function (x,y,z,vPara,mu,t)
    return 0-- -ionMass*vPara/(ionCharge*x)
  end
}
