...
-- total energy vs time dynvector
elcEnergyHistory = DataStruct.DynVector { numComponents = 1, }
-- total perp energy vs time dynvector
...
function applyPositivity(tCurr, myDt, distfElc, distfIon)  -- apply positivity to ions
  ...
  -- compute total elc energy before positivity
  runUpdater(generalIntegralElcCalc, tCurr, myDt, {distfElc, hamilKeElc, bField3d}, {kineticEnergyElc})
  kineticEnergyElc:scale(1/myDt)
  runUpdater(fieldInt3d, tCurr, myDt, {kineticEnergyElc}, {beforePosEnergyVec})
  ...
  -- compute elc perp energy at cells after positivity is applied
  runUpdater(elcEnergyAtCellCalc, tCurr, myDt, {distfElc, jacobianFieldElc, hamilPerpElc},
    {elcPerpEnergyAtCellAfterPositivity, elcPerpEnergyHistory})
  ...
  -- compute elc parallel energy at cells after positivity is applied
  runUpdater(elcEnergyAtCellCalc, tCurr, myDt, {distfElc, jacobianFieldElc, hamilParaElc},
  ...
  -- compute total elc energy after positivity
  runUpdater(generalIntegralElcCalc, tCurr, myDt, {distfElc, hamilKeElc, bField3d}, {kineticEnergyElc})
  kineticEnergyElc:scale(1/myDt)
  runUpdater(fieldInt3d, tCurr, myDt, {kineticEnergyElc}, {afterPosEnergyVec})
...
function rk3(tCurr, myDt)
  ...
  local positivityStatus, stage1Energy = applyPositivity(tCurr, myDt, f1Elc, f1Ion)
  ...
  positivityStatus, stage2Energy = applyPositivity(tCurr, myDt, f1Elc, f1Ion)
  if (positivityStatus == false) then
    return false, netDtSuggested
  end
  -- calculate density of each species for potential solve
  ...
  positivityStatus, stage3Energy = applyPositivity(tCurr, myDt, f1Elc, f1Ion)
  if (positivityStatus == false) then
    return false, netDtSuggested
  end
  -- calculate density of each species for potential solve
  ...
  if Lucee.getRank() == 0 then
    Lucee.logInfo(string.format("Total energy added from positivity = %g", stage3Energy + 2/3*stage2Energy + 1/6*stage1Energy))
  end
