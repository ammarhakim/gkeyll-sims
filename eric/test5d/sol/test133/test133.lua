...
-- mpiexec -n 4 /Users/eshi/Research/gkeyllall/par-opt/gkeyll/gkeyll -i test132.lua -pc_type lu -pc_factor_mat_solver_package superlu_dist
filename = "test133"

-- phase-space decomposition (5d)
phaseDecomp = DecompRegionCalc5D.CartProd { cuts = {9, 36, 2, 1, 1} }
...
function elcTempProfile(x)
  ...
  local inputTempMin = 10

  if (x < xSource) then
    return inputTempMax
  else
    return (inputTempMax-inputTempMin)*math.exp(-(x-xSource)/lambdaSource) + inputTempMin
  end
end
...
function sourceTempProfile(x,y,z)
  ...
  if x < xSource then
    ...
    return 1 - 0.6*((x-xSource)/(X_UPPER-xSource))^2
  end
end
...
function sourceDensityProfile(x,y,z)
  ...
  if math.abs(z) < L_parallel/4 then
    if x < xSource then
      ...
      return math.max(math.exp(-(x-xSource)/lambdaSource), sourceFloor)
    end
  else
    return 0
  end
...
initSourceIon = Updater.EvalOnNodes5D {
   ...
   -- function to use for initialization
   evaluate = function (x,y,z,v,mu,t)
     local ionSourceTemp = 80
		 return math.exp(-ionMass*v^2/(2*ionSourceTemp*eV))*
      math.exp(-math.abs(mu)*bFieldProfile(x)/(ionSourceTemp*eV))
	 end
}
...
function initialIonDensityProfile(x)
  local w = (X_UPPER-X_LOWER)/N_X
  local elcDens = n0

  if (x < X_LOWER + w) then
    -- cubic falloff region
    return elcDens + n0*rho_s^2/elcTemp*18*elcTempProfile(X_LOWER+w)*(w-x+X_LOWER)/w^3 
  elseif (x < xSource) then
    -- constant region
    return elcDens
  elseif (x < X_UPPER - w) then
    -- exponential region
    return elcDens - n0*3*(elcTempProfile(x)-elcTempProfile(xSource))/elcTemp*(rho_s/lambdaSource)^2
  else
    -- cubic falloff region
    return elcDens + n0*rho_s^2/elcTemp*18*elcTempProfile(X_UPPER-w)*(x-X_UPPER+w)/w^3 
  end
end
...
Lucee.logInfo(string.format( 'Source particle rate = %g [1/(s*m^3)]',avgIonDensity:lastInsertedData() ))
