...
-- mpiexec -n 4 /Users/eshi/Research/gkeyllall/par-opt/gkeyll/gkeyll -i test33.lua -pc_type lu -pc_factor_mat_solver_package superlu_dist
...
-- 5-1-16: removed energy part of positivity preserver for comparisons
-- 5-4-16: modified sources

-- phase-space decomposition (5d)
phaseDecomp = DecompRegionCalc5D.CartProd { cuts = {16, 1, 1, 1, 1} }
...
function sourceTempProfile(x)
  return (0.95*math.exp(-(x-sourceX_UH)^2/sourceLambda_UH^2) + 
      0.95*math.exp(-(x-sourceX_EC)^2/sourceLambda_EC^2) + 
      0.05 + 0.05)*sourceTemp
end
...
initSourceIon = Updater.EvalOnNodes5D {
   ...
   evaluate = function (x,y,z,v,mu,t)
      ...
      0.05*sourceAmplitude_UH + 0.05*sourceAmplitude_EC)* 
		  (2*math.pi*sourceTempProfile(x)*eV/ionMass)^(-3/2)*
      math.exp(-ionMass*v^2/(2*sourceTempProfile(x)*eV))*
...
initSourceElc = Updater.EvalOnNodes5D {
   ...
   -- function to use for initialization
   evaluate = function (x,y,z,v,mu,t)
		 return (0.9*sourceAmplitude_UH*math.exp(-(x-sourceX_UH)^2/sourceLambda_UH^2) + 
      0.9*sourceAmplitude_EC*math.exp(-(x-sourceX_EC)^2/sourceLambda_EC^2) + 
      0.1*sourceAmplitude_UH + 0.1*sourceAmplitude_EC)* 
		  (2*math.pi*sourceTempProfile(x)*eV/elcMass)^(-3/2)*
      math.exp(-elcMass*v^2/(2*sourceTempProfile(x)*eV))*
      math.exp(-math.abs(mu)*bFieldProfile(x)/(sourceTempProfile(x)*eV))
	 end
}
...
ionTempParaHistory = DataStruct.DynVector { numComponents = 1, }
elcTempParaHistory = DataStruct.DynVector { numComponents = 1, }
ionTempPerpHistory = DataStruct.DynVector { numComponents = 1, }
elcTempPerpHistory = DataStruct.DynVector { numComponents = 1, }
...
function calcDiagnostics(tCurr, myDt)
  ...
  runUpdater(temperatureElcCalc, tCurr, myDt, {numDensityElc,mom1dir3Elc,mom2dir3Elc,mom1dir4Elc,bField3d},
    {temperatureElc, temperatureParaElc, temperaturePerpElc})
  -- convert to ev
  ...
  runUpdater(temperatureIonCalc, tCurr, myDt, {numDensityIon,mom1dir3Ion,mom2dir3Ion,mom1dir4Ion,bField3d},
    {temperatureIon, temperatureParaIon, temperaturePerpIon})
  -- convert to ev
  temperatureIon:scale(1/eV)
  temperatureParaIon:scale(1/eV)
  temperaturePerpIon:scale(1/eV)
  ...
  -- compute average parallel temperature (volume factor should be removed in post-processing)
  runUpdater(fieldInt3d, tCurr, myDt, {temperatureParaElc}, {elcTempParaHistory})
  runUpdater(fieldInt3d, tCurr, myDt, {temperatureParaIon}, {ionTempParaHistory})
  -- compute average perp temperature (volume factor should be removed in post-processing)
  runUpdater(fieldInt3d, tCurr, myDt, {temperaturePerpElc}, {elcTempPerpHistory})
  runUpdater(fieldInt3d, tCurr, myDt, {temperaturePerpIon}, {ionTempPerpHistory})
end
...
function writeFields(frameNum, tCurr)
  ...
  phiSLowerSmoothed:write( string.format("phiSL_%d.h5", frameNum), tCurr)
  phiSUpperSmoothed:write( string.format("phiSU_%d.h5", frameNum), tCurr)
  ...
  elcTempParaHistory:write( string.format("elcTempParaHistory_%d.h5", frameNum), tCurr)
  ionTempParaHistory:write( string.format("ionTempParaHistory_%d.h5", frameNum), tCurr)
  elcTempPerpHistory:write( string.format("elcTempPerpHistory_%d.h5", frameNum), tCurr)
  ionTempPerpHistory:write( string.format("ionTempPerpHistory_%d.h5", frameNum), tCurr)
