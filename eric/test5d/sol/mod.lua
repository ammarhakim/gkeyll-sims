...
-- mpiexec -n 4 /Users/eshi/Research/gkeyllall/par-opt/gkeyll/gkeyll -i test65.lua -pc_type lu -pc_factor_mat_solver_package superlu_dist
...
n0 = 2*10^18 -- [1/m^3]
-- derived parameters
vtElc   = math.sqrt(3*eV/elcMass) -- only used for grid
...
function ionTempProfile(x,y,z)
  return ionTemp--*initBackgroundProfile(x,y,z)
end
...
initSourceIon = Updater.EvalOnNodes5D {
   ...
   -- function to use for initialization
   evaluate = function (x,y,z,v,mu,t)
		 return math.exp(-ionMass*v^2/(2*ionTemp*eV))*
      math.exp(-math.abs(mu)*bFieldProfile(x)/(ionTemp*eV))
	 end
}
...
scaleInitDistfIon = Updater.SOLInitializeDensity {
  ...
  scaleFactor = 2*math.pi/ionMass,
  evaluate = function(x,y,z,t)
    return n0*initBackgroundProfile(x,y,z)
  end
}
...
fieldSolver = Updater.FemGenPoisson3D {
   onGrid = grid_3d,
   basis = basis_3d,
   ...
   laplacianWeight = rho_s^2/elcTemp,
...
function calcPotential(nIon, nElc, phiOut)
  ...
  local statusPhi = runUpdater(fieldSolver, 0.0, 0.0, {numDensityDelta, nIon}, {phiOut})
...
function calcHamiltonianElc(phiIn, hamilOut)
  ...
  hamilOut:sync()
  totalHamilTime = totalHamilTime + os.clock() - hamilStartTime
end
...
function applyBcToDistF(tCurr, myDt, fElc, fIon, phiIn)
  ...
  fElc:sync()
  fIon:sync()
  totalBcTime = totalBcTime + os.clock() - bcStartTime
end

function calcLenardBernsteinElcElc(tCurr, myDt, fInitial, fFinal)
  local startCollisionTime = os.clock()
  ...
  totalCollisionTime = totalCollisionTime + os.clock() - startCollisionTime
...
function rk3(tCurr, myDt)
  ...
  local arithmeticStartTime = os.clock()
  ...
  totalArithmeticTime = totalArithmeticTime + os.clock() - arithmeticStartTime
  
  if (netStatus == true) then
    return false, netDtSuggested
  end
  -- add on particle source
  local sourceStartTime = os.clock()
  f1Elc:accumulate(myDt, elcSource)
  f1Ion:accumulate(myDt, ionSource)
  totalSourceTime = totalSourceTime + os.clock() - sourceStartTime
...
function advanceFrame(tStart, tEnd, initDt)
  ...
  while tCurr<=tEnd do
    ...
    local rk3StartTime = os.clock()
    status, dtSuggested = rk3(tCurr, myDt)
    totalRk3Time = totalRk3Time + os.clock() - rk3StartTime
...
function calcDiagnostics(tCurr, myDt)
  local startDiagnosticTime = os.clock()
  -- compute electron moments
  ...
  totalDiagnosticTime = totalDiagnosticTime + os.clock() - startDiagnosticTime
end
...
function calcSourceDiagnostics(tCurr, myDt, elcSource, ionSource)
  ...
  -- compute average source temperature
  ...
  Lucee.logInfo(string.format( 'Average source elc temp = %g eV',elcSourceTempHistory:lastInsertedData()/totalVol ))
  ...
  Lucee.logInfo(string.format( 'Average source ion temp = %g eV',ionSourceTempHistory:lastInsertedData()/totalVol ))
...
Lucee.logInfo (string.format("Time per timestep = %g s", totaltime/totalsteps))
...
Lucee.logInfo(string.format("Assembly in poisson solver = %g s (%g %%)", fieldSolver:totalAssemblyTime(), 100*fieldSolver:totalAssemblyTime()/totaltime ) )
...
Lucee.logInfo(string.format("Total time in drag operator = %g s (%g %%)", dragSlvrElc:totalAdvanceTime(), 100*dragSlvrElc:totalAdvanceTime()/totaltime ) )
Lucee.logInfo(string.format("Total time in diff operator = %g s (%g %%)", diffSlvrElc:totalAdvanceTime()+diffSlvrElcIon:totalAdvanceTime(), 100*(diffSlvrElc:totalAdvanceTime()+diffSlvrElcIon:totalAdvanceTime())/totaltime ) )

bcSheathTime = biasedSheathElcCalc:totalAdvanceTime() + biasedSheathIonCalc:totalAdvanceTime()
Lucee.logInfo(string.format("Total time in parallel sheath bc = %g s (%g %%)", bcSheathTime, bcSheathTime*100/totaltime))

generalIntegralTime = generalIntegralIonCalc:totalAdvanceTime() + generalIntegralElcCalc:totalAdvanceTime()
Lucee.logInfo(string.format("Total time in general integral calc = %g s (%g %%)", generalIntegralTime, generalIntegralTime*100/totaltime))

Lucee.logInfo(string.format("Total time in diagnostics = %g s (%g %%)", totalDiagnosticTime, totalDiagnosticTime*100/totaltime))
Lucee.logInfo(string.format("Total time in collisions = %g s (%g %%)", totalCollisionTime, totalCollisionTime*100/totaltime))
Lucee.logInfo(string.format("Total time in bc = %g s (%g %%)", totalBcTime, totalBcTime*100/totaltime))

totalPositivityTime = positivityElcUpdater:totalAdvanceTime() + positivityIonUpdater:totalAdvanceTime()
Lucee.logInfo(string.format("Total time in positivity = %g s (%g %%)", totalPositivityTime, totalPositivityTime*100/totaltime))
Lucee.logInfo(string.format("Total time in rk3 = %g s (%g %%)", totalRk3Time, totalRk3Time*100/totaltime))
Lucee.logInfo(string.format("Total time in hamil = %g s (%g %%)", totalHamilTime, totalHamilTime*100/totaltime))

Lucee.logInfo(string.format("Total time in source = %g s (%g %%)", totalSourceTime, totalSourceTime*100/totaltime))
Lucee.logInfo(string.format("Total time in arithmetic operation = %g s (%g %%)", totalArithmeticTime, totalArithmeticTime*100/totaltime))
