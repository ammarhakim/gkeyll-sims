...
bFieldReduction = 10 -- factor by which to reduce gradients in B
-- derived parameters
...
function elcTempProfile(x)
  ...
  if (x < xSource + 0.04) then
    return inputTempMax
  else
    return (inputTempMax-inputTempMin)*math.exp(-(x-xSource - 0.04)/lambdaSource) + inputTempMin
  end
end
...
function sourceTempProfile(x,y,z)
  ...
  local elcSourceTempMin = 15

  if (x < xSource + 0.04) then
    return elcSourceTempMax
  else
    return (elcSourceTempMax-elcSourceTempMin)*math.exp(-(x-xSource - 0.04)/lambdaSource) + elcSourceTempMin
  end
end
...
function initialIonDensityProfile(x)
  ...
  local elcDens = n0 -- Assume electron density is flat for now
  local w = 2*dxOrig
