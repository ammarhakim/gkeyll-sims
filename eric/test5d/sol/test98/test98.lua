...
elcPerpEnergyHistory = DataStruct.DynVector { numComponents = 1, }
...
elcPerpEnergyAtCellAfterPositivity = DataStruct.Field3D {
   onGrid = grid_3d,
   numComponents = basis_3d:numNodes(),
   ghost = {1, 1},
}
...
elcPerpEnergyAtCellBeforePositivity = DataStruct.Field3D {
   onGrid = grid_3d,
   numComponents = basis_3d:numNodes(),
   ghost = {1, 1},
}
...
elcPerpEnergyAtCellAfterMaxDrag = DataStruct.Field3D {
   onGrid = grid_3d,
   numComponents = basis_3d:numNodes(),
   ghost = {1, 1},
}
...
--beforeCollisionEnergyVec = DataStruct.DynVector { numComponents = 1, }
--afterCollisionEnergyVec = DataStruct.DynVector { numComponents = 1, }
...
function applyIonPositivity(tCurr, myDt, distfIon)
  ...
  --  ionEnergyAtCellBeforePositivity, mom1dir3Ion, numDensityIon}, {fIonPositivityDrag})
  ...
  --  {ionEnergyAtCellAfterMaxDrag, ionEnergyHistory})
...
positivityDragInMuElcUpdater = Updater.SOLPositivityDragCellInMuUpdater {
  onGrid = grid_elc_5d,
  basis5d = basis_elc_5d,
  basis3d = basis_3d,
}
...
function applyElcPositivity(tCurr, myDt, distfElc)
  ...
  -- calculate initial parallel elc energy in each spatial cell before positivity
  ...
  -- calculate initial perp elc energy in each spatial cell before positivity
  runUpdater(elcEnergyAtCellCalc, tCurr, myDt, {distfElc, jacobianFieldElc, hamilPerpElc},
    {elcPerpEnergyAtCellBeforePositivity, elcPerpEnergyHistory})
  ...
  runUpdater(elcEnergyAtCellCalc, tCurr, myDt, {distfElc, jacobianFieldElc, hamilPerpElc},
    {elcPerpEnergyAtCellAfterPositivity})
  -- take a maximum drag step in mu direction
  runUpdater(positivityDragInMuElcUpdater, tCurr, myDt, {distfElc, bField3d, elcPerpEnergyAtCellAfterPositivity,
    elcPerpEnergyAtCellBeforePositivity}, {fElcPositivityDrag})
  -- compute energy in cell after maximum drag step
  runUpdater(elcEnergyAtCellCalc, tCurr, myDt, {fElcPositivityDrag, jacobianFieldElc, hamilPerpElc},
    {elcPerpEnergyAtCellAfterMaxDrag, elcPerpEnergyHistory})
  -- Compute the difference between distf post-positivity and distf post-drag
  ...
  -- to get correct energy at each node
  runUpdater(positivityScaleElcUpdater, tCurr, myDt, {fElcPositivityDelta, elcPerpEnergyAtCellBeforePositivity,
    elcPerpEnergyAtCellAfterPositivity, elcPerpEnergyAtCellAfterMaxDrag}, {distfElc})
...
function calcLenardBernsteinElcElc(tCurr, myDt, fInitial, fFinal)
  ...
  --runUpdater(generalIntegralElcCalc, tCurr, myDt, {fFinal, hamilKeElc, bField3d}, {kineticEnergyElc})
  --runUpdater(fieldInt3d, tCurr, myDt, {kineticEnergyElc}, {beforeCollisionEnergyVec})
  ...
  runUpdater(generalIntegralElcCalc, tCurr, myDt, {fElcCollisions, hamilKeElc, bField3d}, {elcEnergyAtCellDrag})
  ...
  runUpdater(generalIntegralElcCalc, tCurr, myDt, {fElcCollisions, hamilKeElc, bField3d}, {elcEnergyAtCellDiff})
  --runUpdater(elcEnergyAtCellCalc, tCurr, myDt, {fElcCollisions, jacobianFieldElc, hamilKeElc},
  ...
  --runUpdater(generalIntegralElcCalc, tCurr, myDt, {fFinal, hamilKeElc, bField3d}, {kineticEnergyElc})
  --runUpdater(fieldInt3d, tCurr, myDt, {kineticEnergyElc}, {afterCollisionEnergyVec})
  --Lucee.logInfo(string.format("Before = %g, After = %g",beforeCollisionEnergyVec:lastInsertedData(), afterCollisionEnergyVec:lastInsertedData()))
...
function rk3(tCurr, myDt)
  ...
  local positivityElcStatus, stage1ElcEnergy = applyElcPositivity(tCurr, myDt, f1Elc)
  local positivityIonStatus, stage1IonEnergy = applyIonPositivity(tCurr, myDt, f1Ion)
  ...
  local positivityElcStatus, stage2ElcEnergy = applyElcPositivity(tCurr, myDt, f1Elc)
  local positivityIonStatus, stage2IonEnergy = applyIonPositivity(tCurr, myDt, f1Ion)
  ...
  local positivityElcStatus, stage3ElcEnergy = applyElcPositivity(tCurr, myDt, f1Elc)
  local positivityIonStatus, stage3IonEnergy = applyIonPositivity(tCurr, myDt, f1Ion)
...
if Lucee.IsRestarting then
  ...
  --applyIonPositivity(0.0, 0.0, fIon)
