...
-- 9-21-2015: added deviceScale variable to scale R0 and a
-- -pc_type lu -pc_factor_mat_solver_package superlu_dist

-- phase-space decomposition
phaseDecomp = DecompRegionCalc4D.CartProd { cuts = {1, 2, 4, 1} }
...
deviceScale = 100 -- used to scale R0 and a
B0 = 1.91   -- [T]
R0 = 1.313*deviceScale  -- [m]
a  = 0.4701*deviceScale -- [m]
...
Y_LOWER = -deltaR/2
Y_UPPER = deltaR/2
VPARA_UPPER = 6*vtKinetic
VPARA_LOWER = -VPARA_UPPER
MU_LOWER = 0
MU_UPPER = 18*kineticMass*vtKinetic*vtKinetic/(B0)
...
copy2dTo4d = Updater.CopyNodalFields2D_4D {
   onGrid = grid_4d,
...
-- cosine basis function stored on a dg field
cosineField = DataStruct.Field2D {
   onGrid = grid_2d,
   numComponents = basis_2d:numNodes(),
   ghost = {1, 1},
}
-- sine basis function stored on a dg field
sineField = DataStruct.Field2D {
   onGrid = grid_2d,
   numComponents = basis_2d:numNodes(),
   ghost = {1, 1},
}
-- Updater to initialize cosine basis field
initCosineField = Updater.EvalOnNodes2D {
  onGrid = grid_2d,
  basis = basis_2d,
  shareCommonNodes = false,
  evaluate = function (x,y,t)
...
cosineField:sync()
-- Updater to initialize sine basis field
initSineField = Updater.EvalOnNodes2D {
  onGrid = grid_2d,
  basis = basis_2d,
  shareCommonNodes = false,
  evaluate = function (x,y,t)
...
sineField:sync()
-- dynvectors to store weight of smoothed potential
cosineWeights = DataStruct.DynVector { numComponents = 1, }
sineWeights = DataStruct.DynVector { numComponents = 1, }
-- Updater to project potential on sine and cosine fields
modeProjectionCalc = Updater.IntegrateFieldProduct2D {
  onGrid = grid_2d,
  basis = basis_2d,
}
-- dynvectors to normalize projections
cosineNormWeights = DataStruct.DynVector { numComponents = 1, }
sineNormWeights = DataStruct.DynVector { numComponents = 1, }
-- (one time only) compute the normalizations
runUpdater(modeProjectionCalc, 0.0, 0.0, {cosineField, cosineField}, {cosineNormWeights})
runUpdater(modeProjectionCalc, 0.0, 0.0, {sineField, sineField}, {sineNormWeights})
...
function scalePertrubation(curr, dt, distf, targetEnergy)
  -- scale perturbed part of distribution function:
  -- Subtract out fluctuating component from f
  fFluctuating:copy(distf)
  fFluctuating:accumulate(-1.0, fBackground)
  distf:accumulate(-1.0, fFluctuating)
...
function advanceFrame(tStart, tEnd, initDt)
  ...
  while tCurr<=tEnd do
    ...
    if (status == false) then
      ...
      scalePertrubation(tCurr, myDt, f, initialFieldEnergy)
      tCurr = tCurr + myDt
      myDt = dtSuggested
      step = step + 1
