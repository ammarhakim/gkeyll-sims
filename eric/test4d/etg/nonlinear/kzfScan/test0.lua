...
function perturbDensityProfile(x,y)
  ...
  return 1e-3*(vtKinetic/omega_s)/L_T*math.cos(ky_min*y)*math.exp(-(x-x0)^2/(2*sigma^2))
end
...
phiCalc = Updater.ETGAdiabaticPotentialUpdater {
  onGrid = grid_2d,
  basis = basis_2d,
  kzfTimesRhoSquared = kzfTimesRhoSquared,
