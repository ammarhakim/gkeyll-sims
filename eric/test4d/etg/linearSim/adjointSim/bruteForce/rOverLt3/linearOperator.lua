...
-- 7-9-2015: from brute.lua, and uses eval on nodes for background distribution
-- 7-9-2015: note that MU_UPPER = math.min(8, 4*math.sqrt(N_MU/2))*kineticMass*vtKinetic*vtKinetic/(B0)
...
fBackground:copy(f)
-- Write background f
fBackground:write( string.format("fBackground_%d.h5", 0), 0)
...
matrixConstructor = Updater.ConstructLinearOperatorMatrix4D {
  ...
  filename = "linearOperatorMatrix3.mtx",
}
