...
-- NEW INITIALIZATION
-- Same as test0, but with initial conditions shifted over
...
function perturbDensityProfile(x,y)
  ...
  if x > x0 then
    return n0*1e-4*(vtKinetic/omega_s)/L_T*math.cos(ky_min*y)--*math.exp(-(x-X_UPPER)^2/(2*sigma^2))
  else
    return n0*1e-4*(vtKinetic/omega_s)/L_T*math.cos(ky_min*y)--*math.exp(-(x-X_LOWER)^2/(2*sigma^2))
  end
end
...
--applyBcToBackgroundPotential(phi2dSmoothed)
