...
-- OLD INITIALIZATION
-- Same as test1, but with more resolution in velocity space
-- Also uses zeroFlux bc
...
Y_LOWER = -deltaR/2
Y_UPPER = deltaR/2
VPARA_LOWER = -4*vtKinetic
VPARA_UPPER = 4*vtKinetic
MU_LOWER = 0
...
function perturbDensityProfile(x,y)
  ...
  return n0*1e-4*(vtKinetic/omega_s)/L_T*math.cos(ky_min*y)*math.exp(-(x-x0)^2/(2*sigma^2))
end
