-- File to test field smoothing operation
...
numDensityElc:scale(2*math.pi/elcMass)
numDensityElc:write( string.format("n_0.h5"))
...
runUpdater(numDensityCalc, 0, 0, {f, bField2D}, {numDensityElc})
numDensityElc:scale(2*math.pi/elcMass)
numDensityElc:write( string.format("n_1.h5"))
runUpdater(totalPtclCalc, 0, 0, {numDensityElc}, {totalPtcl})
print( string.format("Recomputed Numerical N = %e",totalPtcl:lastInsertedData()) )

-- NEW STUFF
initN = Updater.ProjectOnNodalBasis2D {
   onGrid = grid_2d,
   basis = basis_2d,
   shareCommonNodes = false,
   -- function to use for initialization
   evaluate = function (x,y,v,mu,t)
		 return math.sin(y/(deltaR/8))
	 end
}
runUpdater(initN, 0.0, 0.0, {}, {numDensityElc})
numDensityElc:write( string.format("nTest_0.h5"), 0)

numDensitySmoothed = numDensityElc:duplicate()
-- Smooth out field
smoothN = Updater.SimpleSmoothToC02D {
   onGrid = grid_2d,
   basis = basis_2d,
}
runUpdater(smoothN, 0.0, 0.0, {numDensityElc}, {numDensitySmoothed})
numDensitySmoothed:write( string.format("nTest_1.h5"), 1)
