...
function fProfile(x,y,v,mu)
  return n0*(2*math.pi*kineticTempProfile(x)*eV/kineticMass)^(-3/2)*
...
-- Fill out jacobian
runUpdater(copy2dTo4d, 0.0, 0.0, {bField2d}, {jacobianField})
jacobianField:scale(kineticMass*kineticMass)
...
phi2d_smoothed = phi2d:duplicate()
...
function calcDiagnostics(curr, dt)
  runUpdater(fieldEnergyCalc, curr, dt, {phi2d_smoothed}, {fieldEnergy})
end
...
function rk3(tCurr, myDt)
  ...
  -- smooth potential
  runUpdater(smoothCalc, tCurr, myDt, {phi2d}, {phi2d_smoothed})
  -- Compute hamiltonian
  calcHamiltonian(hamilKE, phi2d_smoothed, hamil)
...
-- smooth potential
runUpdater(smoothCalc, 0.0, 0.0, {phi2d}, {phi2d_smoothed})
-- Compute hamiltonian
calcHamiltonian(hamilKE, phi2d_smoothed, hamil)

-- Store background phi
phi2dBackground:copy(phi2d_smoothed)
