...
bParY = DataStruct.Field4D {
   onGrid = grid,
   numComponents = numDgNodesPerCell,
   ghost = {2, 2},
}
function initbParY(x, y, vPar, mu)
  return 1
end
-- updater to initialize bParY
initbParYUpdater = Updater.EvalOnNodes4D {
   onGrid = grid,
   basis = basis,
   shareCommonNodes = false,
   evaluate = function (x,y,vPar,mu,t)
		 return initbParY(x,y,vPar,mu)
	 end
}
initbParYUpdater:setOut( {bParY} )
-- initialize
initbParYUpdater:advance(0.0) -- time is irrelevant
