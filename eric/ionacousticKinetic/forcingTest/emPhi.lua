-- Tests A_parallel implementation by using phi nonzero, A zero
-- that gives same resulting E-field as emA.lua
...
distf0Elc = DataStruct.Field2D {
   onGrid = gridElc,
   numComponents = basisElc:numNodes(),
   ghost = {1, 1},
}
distDeltafElc = DataStruct.Field2D {
   onGrid = gridElc,
   numComponents = basisElc:numNodes(),
   ghost = {1, 1},
}
initDistf0Elc = Updater.EvalOnNodes2D {
   onGrid = gridElc,
   -- basis functions to use
   basis = basisElc,
   -- are common nodes shared?
   ...
   evaluate = function(x,y,z,t)
      local elcThermal = math.sqrt(elcTemp*Lucee.ElementaryCharge/elcMass)
		  local k = knumber
		  local nHat = initNumDens
		  return maxwellian(initNumDens, elcMass, elcThermal, y)
	   end
}
runUpdater(initDistf0Elc, 0.0, 0.0, {}, {distf0Elc})
...
function calcDiagnostics(curr, dt)
  ...
  distDeltafElc:clear(0.0)
  distDeltafElc:accumulate(1.0, distf0Elc)
  distDeltafElc:accumulate(-1.0, distfElc)

  -- Copy A to 2D, DG field
...
forcePhiUpdater = Updater.EvalOnNodes1D {
   ...
   basis = basis_1d,
   -- are common nodes shared?
   shareCommonNodes = true, -- phi is continuous
   -- function to use for initialization
   evaluate = function(x,y,z,t)
      ...
      local phi_1 = alpha*elcTemp
		  local k = knumber
		  local theta = 0
		  return phi_1*math.cos(k*x + theta)
	   end
}

forceAUpdater = Updater.EvalOnNodes1D {
   onGrid = grid_1d,
   -- basis functions to use
   basis = basis_1d,
   -- are common nodes shared?
...
function rk3(tCurr, myDt)
   ...
   -- Compute phi
   runUpdater(forcePhiUpdater, tCurr, myDt, {}, {phi1d})
   -- Compute A-parallel
   runUpdater(forceAUpdater, tCurr, myDt, {}, {aParallel1d})
   ...
   -- Compute phi
   runUpdater(forcePhiUpdater, tCurr, myDt/2, {}, {phi1d})
   -- Compute A-parallel
   runUpdater(forceAUpdater, tCurr, myDt/2, {}, {aParallel1d})
   -- Copy continuous field back to discontinuous field
   calcDiscontinuousField(tCurr, myDt, aParallel1d, aParallel1dDg)
   -- Compute projection of A(x)^2
   runUpdater(aSquaredCalc, tCurr, myDt, {aParallel1dDg}, {aSquared1dDg})
...
function writeFields(frameNum, tCurr)
  ...
  distDeltafElc:write(string.format("distDeltafElc_%d.h5", frameNum), tCurr)
...
runUpdater(forcePhiUpdater, 0.0, 0.0, {}, {phi1d})
-- Compute A-parallel
runUpdater(forceAUpdater, 0.0, 0.0, {}, {aParallel1d})
-- Copy continuous field back to discontinuous field
