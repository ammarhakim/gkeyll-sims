...
-- CG fields to store Phi(x) on 2D grids
...
hamilElcCalc = Updater.NonLinEmGke1DHamil {
  onGrid = gridElc,
  basis = basisElc,
  charge = elcCharge,
  mass = elcMass,
...
hamilIonCalc = Updater.NonLinEmGke1DHamil {
  onGrid = gridIon,
  basis = basisIon,
  charge = ionCharge,
  mass = ionMass,
...
function calcHamiltonianElc(curr, dt, phiIn, aParallel1dIn, hamilOut)
   -- clear out fields (is this needed?)
   hamilOut:clear(0.0)
   aParallel2dElc:clear(0.0)
   phi2dElc:clear(0.0)
   -- Compute projection of aParallel and phi onto 2D grid
   copyCont1DTo2D(copyTo2DElc, curr, dt, aParallel1dIn, aParallel2dElc)
   copyCont1DTo2D(copyTo2DElc, curr, dt, phiIn, phi2dElc)
   runUpdater(hamilElcCalc, curr, dt, {phi2dElc,aParallel2dElc}, {hamilOut})
end
-- compute hamiltonian for ions
function calcHamiltonianIon(curr, dt, phiIn, aParallel1dIn, hamilOut)
   -- clear out fields (is this needed?)
   hamilOut:clear(0.0)
   ...
   copyCont1DTo2D(copyTo2DIon, curr, dt, phiIn, phi2dIon)
   runUpdater(hamilIonCalc, curr, dt, {phi2dIon,aParallel2dIon}, {hamilOut})
end
