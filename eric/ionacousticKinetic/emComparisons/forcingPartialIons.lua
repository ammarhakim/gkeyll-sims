...
-- Currently modified to use the A_in/A_out updater to compute A_out
-- Partial updater has been modified to be equivalent to the full updater
...
print(string.format('kPerp = %g',kPerp))
...
aParallel1dDgOld = DataStruct.Field1D {
   onGrid = grid_1d,
   numComponents = basis_1d:numNodes(),
   ghost = {1, 1},
}
aParallel1dDgZero = DataStruct.Field1D {
   onGrid = grid_1d,
   numComponents = basis_1d:numNodes(),
   ghost = {1, 1},
}
aParallel1dDgZero:clear(0.0)
...
function rk3(tCurr, myDt)
   ...
   -- Compute A-parallel
   runUpdater(electromagneticACalc, tCurr, myDt, {mom0Elc, mom0Ion, mom1Elc, mom1Ion, aParallel1dDgZero}, {aParallel1dDg})
   ...
   aParallel1dDgOld:copy(aParallel1dDg)
...
function advanceFrame(tStart, tEnd, initDt)
   ...
   while tCurr<=tEnd do
      ...
      aParallel1dDgOldDup:copy(aParallel1dDgOld)
      ...
      if (status == false) then
	      ...
	      aParallel1dDgOld:copy(aParallel1dDgOldDup)
...
aParallel1dDgOld:clear(0.0)
-- Compute A-parallel
runUpdater(electromagneticACalc, 0.0, 0.0, {mom0Elc, mom0Ion, mom1Elc, mom1Ion, aParallel1dDgZero}, {aParallel1dDg})
...
-- Store value of aParallel1dDgOld in case we need to revert to it after a time step
aParallel1dDgOldDup = aParallel1dDgOld:duplicate()
writeFields(0, 0.0)
